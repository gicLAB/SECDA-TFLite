...
SC_MODULE(SUBMODULENAME) {
  ...
  int Quantised_Multiplier(int x, int qm, int shift) {
    ...
    sc_int<32> tempdel_1;
    tempdel_1 = (val_2 + POS) / DIVMAX;
    if (val_2 < 0) tempdel_1 = (val_2 + NEG) / DIVMAX;
    sc_int<32> val_3 = tempdel_1;
    val_3 = val_3 >> pr;
    sc_int<32> tempdel_2 = tempdel_1 & msk;
    sc_int<32> tempdel_3 = (tempdel_1 < 0) & 1;
    sc_int<32> tempdel_4 = sm + tempdel_3;
    sc_int<32> tempdel_5 = ((tempdel_2 > tempdel_4) & 1);
    sc_int<32> result_32 = val_3 + tempdel_5;
    return result_32;
  }
...
struct hw_submodule_var_array {
  hw_submodule_vars vars_0;
  hw_submodule_vars vars_1;
  SUBMODULENAME HW0;
  SUBMODULENAME HW1;

#ifndef __SYNTHESIS__
  hw_submodule_var_array()
      : vars_0(16, 0), HW0("HW0"), vars_1(16, 1), HW1("HW1") {}
#else
  hw_submodule_var_array() : vars_0(16), HW0("HW0"), vars_1(16), HW1("HW1") {}
#endif

  hw_submodule_vars &operator[](int i) {
    if (i == 0) return vars_0;
    else if (i == 1) return vars_1;
    else return vars_0;
  }
  ...
  void input_fifo_write(int i, int val) {
    if (i == 0) vars_0.input_fifo.write(val);
    else if (i == 1) vars_1.input_fifo.write(val);
    else vars_0.input_fifo.write(val);
  }

  int output_fifo_read(int i) {
    if (i == 0) return vars_0.output_fifo.read();
    else if (i == 1) return vars_1.output_fifo.read();
    else return vars_0.output_fifo.read();
  }
};
