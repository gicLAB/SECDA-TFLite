...
// #define RECORD
#define WEIGHT_PRELOAD
// #define INPUT_PRELOAD
...
public:
  ...
  TfLiteStatus Init(TfLiteContext *context,
    ...
    if (!dparams.init) {
      ...
      DTOG(std::cerr << "MM2IM Accelerator v11");
#ifdef ACC_NEON
...
#if defined(RUN_CPU_TCONV) || defined(RECORD)
...
#if defined(SYSC) || defined(RECORD)
      saveMatrixCSV("aData/mm2im/" + std::to_string(associated_nodes[i]) +
                        "_out_acc.csv",
                    output_data, scratch_cols, scratch_rows);
#endif
