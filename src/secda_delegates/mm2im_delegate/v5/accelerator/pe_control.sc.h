...
void ACCNAME::set_row_PEs(int crow) {
  // cout << "set_row_PEs: " << crow << endl;
  for (int i = 0; i < PE_COUNT; i++) {
#pragma HLS unroll
    vars[i].crow.write(crow);
    vars[i].num_rows.write(number_of_rows);
    vars[i].process_cal.write(true);
  }
  wait();
  for (int i = 0; i < PE_COUNT; i++) {
#pragma HLS unroll
    while (!vars[i].process_cal_done) wait();
    vars[i].process_cal.write(false);
  }
}
...
//   for (int i = 0; i < PE_COUNT; i++) {
// #pragma HLS unroll
//     loop = loop || vars[i].out_fifo.num_available();
//   }
//   DWAIT(2);
//   return loop;
// }

void ACCNAME::col_indices_write(int data, bool tlast) {
  for (int i = 0; i < PE_COUNT; i++) {
#pragma HLS unroll
...
void ACCNAME::out_indices_write(int data, bool tlast) {
  for (int i = 0; i < PE_COUNT; i++) {
#pragma HLS unroll
