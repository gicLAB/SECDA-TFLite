...
void PE::Fill_Out() {
  wait();
  while (1) {
    ADATA outdex = out_indices_fifo.read();
    int pouts = 0;
    while (!outdex.tlast) {
#pragma HLS PIPELINE II = 1
      int dex = outdex.data % PE_ACC_BUF_SIZE;
      out_offset[pouts++] = dex;
      outdex = out_indices_fifo.read();
      DWAIT();
    }
    for (int i = 0; i < pouts; i++) {
      int dex = out_offset[i];
