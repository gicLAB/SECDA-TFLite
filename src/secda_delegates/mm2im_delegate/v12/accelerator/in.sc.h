...
#pragma HLS resource core=AXI4LiteS metadata="-bus_bundle slv0" variable=vars.vars_7.sendS
  ...
  while (1) {
    ...
    opcode op = opcode(in_fifo.read().data.to_uint());
    if (op.load_con) {
      inS.write(2);
      wait();
      depth = in_fifo.read().data;
      row_size = in_fifo.read().data;
      cols_per_filter = in_fifo.read().data;
      inp_rows = in_fifo.read().data;
      ra = in_fifo.read().data;
      oh = in_fifo.read().data;
      ow = in_fifo.read().data;
      kernel_size = in_fifo.read().data;
      stride_x = in_fifo.read().data;
      stride_y = in_fifo.read().data;
      pt = in_fifo.read().data;
      pl = in_fifo.read().data;
      width_col = in_fifo.read().data;
      DWAIT(13);
    }
