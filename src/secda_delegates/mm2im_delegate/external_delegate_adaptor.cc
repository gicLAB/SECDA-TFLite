...
TfLiteDelegate* CreateMM2IMDelegateFromOptions(char** options_keys,
                                               char** options_values,
                                               size_t num_options) {
  MM2IMDelegateOptions options = TfLiteMM2IMDelegateOptionsDefault();

  // Parse key-values options to MM2IMDelegateOptions by mimicking them as
  // command-line flags.
  std::vector<const char*> argv;
  argv.reserve(num_options + 1);
  constexpr char kMM2IMDelegateParsing[] = "mm2im_delegate_parsing";
  argv.push_back(kMM2IMDelegateParsing);
  ...
  TFLITE_LOG(INFO) << "MM2IM delegate: allowed_builtin_code set to "
                   << options.allowed_builtin_code << ".";
  TFLITE_LOG(INFO) << "MM2IM delegate: error_during_init set to "
                   << options.error_during_init << ".";
  TFLITE_LOG(INFO) << "MM2IM delegate: error_during_prepare set to "
                   << options.error_during_prepare << ".";
  TFLITE_LOG(INFO) << "MM2IM delegate: error_during_invoke set to "
                   << options.error_during_invoke << ".";

  return TfLiteMM2IMDelegateCreate(&options);
}
...
TFL_CAPI_EXPORT TfLiteDelegate* tflite_plugin_create_delegate(
  ...
  return tflite::tools::CreateMM2IMDelegateFromOptions(
      options_keys, options_values, num_options);
}

TFL_CAPI_EXPORT void tflite_plugin_destroy_delegate(TfLiteDelegate* delegate) {
  TfLiteMM2IMDelegateDelete(delegate);
}
