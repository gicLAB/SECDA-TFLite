...
SC_MODULE(ACCNAME) {
  ...
  ACCNAME(sc_module_name name_) : sc_module(name_), vars() {
    ...
    SC_CTHREAD(Output_Pattern_Decoder, clock);
    reset_signal_is(reset, true);
...
#pragma HLS array_partition variable = out_reg complete
...
#pragma HLS array_partition variable = out_done_reg complete
