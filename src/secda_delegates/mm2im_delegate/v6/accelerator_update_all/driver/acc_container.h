...
struct mm2im_times {
...
#ifdef ACC_PROFILE
    cout << "================================================" << endl;
    prf_out(TSCALE, t_tconv);
    prf_out(TSCALE, p_store);
    prf_out(TSCALE, p_load_wgt);
    prf_out(TSCALE, p_load_inp);
    prf_out(TSCALE, p_start_sched);
    prf_out(TSCALE, p_load_config);
  ...
  void save_prf() {
    ...
    prf_file_out(TSCALE, p_load_config, file);
    prf_file_out(TSCALE, p_load_wgt, file);
    prf_file_out(TSCALE, p_load_inp, file);
    prf_file_out(TSCALE, p_start_sched, file);
    prf_file_out(TSCALE, p_store, file);
    prf_file_out(TSCALE, t_tconv, file);
    file.close();
#endif
  }
};
