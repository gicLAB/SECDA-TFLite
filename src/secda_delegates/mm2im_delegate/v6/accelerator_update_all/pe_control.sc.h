...
#pragma HLS unroll
    ...
    vars[i].crow.write(crow_reg[i]);
    vars[i].num_rows.write(num_rows_reg[i]);
  }
}

void ACCNAME::update_reads() {
#pragma HLS inline OFF
  for (int i = 0; i < PE_COUNT; i++) {
#pragma HLS unroll
    compute_done_reg[i] = vars[i].compute_done.read();
    out_done_reg[i] = vars[i].out_done.read();
    wgt_loaded_reg[i] = vars[i].wgt_loaded.read();
    send_done_reg[i] = vars[i].send_done.read();
    process_cal_done_reg[i] = vars[i].process_cal_done.read();
  }
}
...
#pragma HLS inline OFF
  ...
  wait();
  for (int i = 0; i < limit; i++) {
    loop = loop || !compute_done_reg[i] || !out_done_reg[i];
  }
  DWAIT(2);
  return loop;
}
