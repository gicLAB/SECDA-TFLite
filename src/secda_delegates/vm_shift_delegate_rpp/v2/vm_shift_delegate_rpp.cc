...
public:
  ...
  TfLiteStatus Init(TfLiteContext *context,
    ...
    if (!dparams.init) {
      ...
      std::cout << "Vector Shift MAC RPP (v2)";
#ifdef ACC_NEON
      std::cout << " with Neon";
#endif
      std::cout << std::endl;
  ...
  TfLiteStatus Prepare(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      preload_weights(filter->data.int8, dims, wb0[i], wb1[i], wb2[i], wb3[i],
                      wt_sum1[i], wt_sum2[i], wt_sum3[i], wt_sum4[i],inpZeroPoint, biases[i]);


      // cout << "i=" << i << endl;
      // int b1 = (wb0[i][0] & 0x00000007);
      // int b2 = (wb0[i][0] & 0x00000070)>>4;
      // int b3 = (wb0[i][1] & 0x00000007);
      // int b4 = (wb0[i][1] & 0x00000070)>>4;
      // int b5 = (wb0[i][2] & 0x00000007);
      // int b6 = (wb0[i][2] & 0x00000070)>>4;
      // int b7 = (wb0[i][3] & 0x00000007);
      // int b8 = (wb0[i][3] & 0x00000070)>>4;

      // cout << b1 << " " << b2 << " " << b3 << " " << b4 << " " << b5 << " " << b6 << " " << b7 << " " << b8 << endl;

      // cout << "exiting prepare" << endl;
  ...
  TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) override {
    ...
    // int b1 = (wb0[i][0] & 0x00000007);
    // int b2 = (wb0[i][0] & 0x00000070)>>4;
    // int b3 = (wb1[i][0] & 0x00000007);
    // int b4 = (wb1[i][0] & 0x00000070)>>4;
    // int b5 = (wb2[i][0] & 0x00000007);
    // int b6 = (wb2[i][0] & 0x00000070)>>4;
    // int b7 = (wb3[i][0] & 0x00000007);
    // int b8 = (wb3[i][0] & 0x00000070)>>4;

    // cout << b1 << " " << b2 << " " << b3 << " " << b4 << " " << b5 << " " << b6 << " " << b7 << " " << b8 << endl;

    // cout << "exiting Eval" << endl;
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      // int a1 = (wb_0[0] & 0x00000007);
      // int a2 = (wb_0[0] & 0x00000070)>>4;
      // int a3 = (wb_0[0] & 0x00000700)>>8;
      // int a4 = (wb_0[0] & 0x00007000)>>12;
      // int a5 = (wb_0[0] & 0x00070000)>>16;
      // int a6 = (wb_0[0] & 0x00700000)>>20;
      // int a7 = (wb_0[0] & 0x07000000)>>24;
      // int a8 = (wb_0[0] & 0x70000000)>>28;
...
TfLiteDelegate *TfLiteVMDelegateCreate(const VMDelegateOptions *options) {
  ...
  return tflite::TfLiteDelegateFactory::CreateSimpleDelegate(std::move(vm),kTfLiteDelegateFlagsAllowDynamicTensors); // rpp-kTfLiteDelegateFlagsAllowDynamicTensors --for dynamic tensor
}
