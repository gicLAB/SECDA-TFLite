...
struct vm_times {
  ...
  duration_ns p_send_weights;
  duration_ns p_set_results;
  duration_ns p_start_compute;
  duration_ns p_receive_results;
  ...
  void print() {
    ...
    prf_out(TSCALE, p_send_weights);
    prf_out(TSCALE, p_set_results);
    prf_out(TSCALE, p_start_compute);
    prf_out(TSCALE, p_receive_results);
  ...
  void save_prf() {
    ...
    // prf_file_out(TSCALE, p_send_weights, file);
    // prf_file_out(TSCALE, p_set_results, file);
    // prf_file_out(TSCALE, p_store, file);
    // prf_file_out(TSCALE, p_start_compute, file);
    // prf_file_out(TSCALE, p_receive_results, file);
...
//========================Bit shift specific code========================//

/// @brief simple log2 function
/// @param x should be power of 2
/// @return output is log2(x)
...
int8_t roundToNearestPowerOf2(int8_t x) {
    // if (x <= 0) return 0;
    int8_t wgt = x;
    int sign = (x > 0) ? 1 : -1;
    int abs_x  = abs(x);
    ...
    if ((abs_x - (result >> 1)) < (result - abs_x)) {
        result = (result >> 1);
    } else {
        result = result;
    }

    result = log2_custom(result);

// Handling signs -- Positive numbers
// sign 1 indicates positive number
  if (sign == 1 && x != 0)
    return (int8_t)result;


// Handling signs -- Negative numbers
// -128 i.e -7 will be remapped to zero for BITSHIFT_4BITWGT_BASEL1
  if (sign==-1 && x != 0) return (int8_t)(result+8);


// Handling Zero (Act_zero) either as 1 or 15
  // define act_zero as 1
  // shift 0 analogus multiplication with 1
  // this is just an exmple in real case scenario
  // there will be no zero in the weight matrix
  if (wgt==0) 
  {
    return 0;
  }
...
int noOfWgtToPack = 8; // noOfWgtToPack weights to pack in one 32 bit word

void compress_weight_buffer_packed (int* dims, vector<int8_t>& wb)
{
  int width = dims[0];
  ...
  for (i = 0; i < ((w/4) *d); i+=8)
  {
    ...
    wb[write_counter+0] = res >> 0;
    wb[write_counter+1] = res >> 8;
    wb[write_counter+2] = res >> 16;
    wb[write_counter+3] = res >> 24;

    // int b1 = res & 0x00000007;
    // int b2 = (res & 0x00000070)>>4;
    // int b3 = (res & 0x00000700)>>8;
    // int b4 = (res & 0x00007000)>>12;
    // int b5 = (res & 0x00070000)>>16;
    // int b6 = (res & 0x00700000)>>20;
    // int b7 = (res & 0x07000000)>>24;
    // int b8 = (res & 0x70000000)>>28;

    // int b1 = wb[write_counter+0] & 0x00000007;
    // int b2 = (wb[write_counter+0] & 0x00000070)>>4;
    // int b3 = (wb[write_counter+1] & 0x00000700)>>8;
    // int b4 = (wb[write_counter+1] & 0x00007000)>>12;
    // int b5 = (wb[write_counter+2] & 0x00070000)>>16;
    // int b6 = (wb[write_counter+2] & 0x00700000)>>20;
    // int b7 = (wb[write_counter+3] & 0x07000000)>>24;
    // int b8 = (wb[write_counter+3] & 0x70000000)>>28;
    
    // int b1 = wb[write_counter+0] & 0x00000007;
    // int b2 = (wb[write_counter+0] & 0x00000070)>>4;
    // int b3 = (wb[write_counter+1] & 0x00000007);
    // int b4 = (wb[write_counter+1] & 0x00000070)>>4;
    // int b5 = (wb[write_counter+2] & 0x00000007);
    // int b6 = (wb[write_counter+2] & 0x00000070)>>4;
    // int b7 = (wb[write_counter+3] & 0x00000007);
    // int b8 = (wb[write_counter+3] & 0x00000070)>>4;

    // cout << b1 << " " << b2 << " " << b3 << " " << b4 << " " << b5 << " " << b6 << " " << b7 << " " << b8 << endl;

    // cout << "exiting compress_weight_buffer_packed" << endl;
  ...
  for (int i = 0; i < w / 4; i++) {
    ...
    for (int j = 0; j < d; j++) {
      if (j < depth) {
        ...
        s3 += w3;
// Bit shift specific code        
        w0 = roundToNearestPowerOf2(w0);
        w1 = roundToNearestPowerOf2(w1);
        w2 = roundToNearestPowerOf2(w2);
        w3 = roundToNearestPowerOf2(w3);
        // cout << (int) (w0 & 0x07) << " ";
        wb0.push_back(w0);
        wb1.push_back(w1);
        wb2.push_back(w2);
    ...
    // cout << endl;
    // cout << " exititng prep_load_weights" << endl;
    // exit(0);
  ...
  compress_weight_buffer_packed (dims, wb0);
  compress_weight_buffer_packed (dims, wb1);
  compress_weight_buffer_packed (dims, wb2);
  compress_weight_buffer_packed (dims, wb3);

  // int b1 = wb0[0] & 0x00000007;
  // int b2 = (wb0[0] & 0x00000070)>>4;
  // int b3 = (wb0[1] & 0x00000007);
  // int b4 = (wb0[1] & 0x00000070)>>4;
  // int b5 = (wb0[2] & 0x00000007);
  // int b6 = (wb0[2] & 0x00000070)>>4;
  // int b7 = (wb0[3] & 0x00000007);
  // int b8 = (wb0[3] & 0x00000070)>>4;
...
#if 0
void prep_load_weights(const int8_t *weight_data, int *dims, vector<int8_t> &wb0,
                     vector<int8_t> &wb1, vector<int8_t> &wb2,
                     vector<int8_t> &wb3, vector<int> &wt_sum1,
                     vector<int> &wt_sum2, vector<int> &wt_sum3,
                     vector<int> &wt_sum4) {
