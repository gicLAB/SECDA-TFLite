...
#define ACCNAME VM_SHIFT_RPP_INT8_V4_0
...
#elif defined(MUL88)
#define PROD_DATA_WIDTH PROD_DATA_WIDTH_8x8
#define WGT_BRAM_DATAWIDTH 32
...
struct config_packet {
  ...
  config_packet(sc_fifo_in<DATA> *din) {
    ...
    ALOG("Time: " << sc_time_stamp());
    depth = din->read().data;
    wgt_rows = din->read().data;
    inp_rows = din->read().data;
  }
};
