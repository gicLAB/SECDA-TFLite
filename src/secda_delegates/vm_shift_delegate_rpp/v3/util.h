...
void QuantizeMultiplier_rpp(double double_multiplier, int32_t* quantized_multiplier,
                        int* shift) {
...
#endif
  auto q_new = q - 0.5;
  // find the nearest power of two from a double value
  q_new = std::round(std::log2(q_new));

  // q_new = std::pow(2, q_new);

  // q_new = q - 0.5 - q_new;
  *quantized_multiplier = static_cast<int32_t>(q_new*(-1));
  // *quantized_multiplier = static_cast<int32_t>(q_fixed);
}

// Per-axis & per-tensor
static TfLiteStatus PopulateConvolutionQuantizationParams_rpp(
  ...
  for (int i = 0; i < num_channels; ++i) {
    ...
    int32_t significand;
    int channel_shift;
    QuantizeMultiplier_rpp(effective_output_scale, &significand, &channel_shift);
