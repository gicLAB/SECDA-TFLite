...
public:
  ...
  TfLiteStatus Init(TfLiteContext *context,
    ...
    if (!dparams.init) {
      ...
      std::cout << "Vector Shift MAC RPP (v3)";
#ifdef ACC_NEON
      std::cout << " with Neon";
#endif
      std::cout << std::endl;
  ...
  TfLiteStatus Prepare(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      for (int j = 0; j < channels_out; j++)
        crx[i][j] = (int8_t)data->per_channel_output_shift.data()[j]*(-1);

      // int crf_max = 0;
      // int crf_min = 0;
      // for (int j = 0; j < channels_out; j++)
      // {
      //   if (crf[i][j]>crf_max) crf_max = crf[i][j];
      //   if (crf[i][j]<crf_min) crf_min = crf[i][j];
      // }
      // cout << "Before crf_max: " << crf_max << endl;
      // cout << "Before crf_min: " << crf_min << endl;

      // We need to make crf[i][j] as our second shift
      TF_LITE_ENSURE_STATUS(PopulateConvolutionQuantizationParams_rpp(
          context, input, filter, bias, output, &crf[i][0], channels_out));

      // int crx_max = 0;
      // int crx_min = 0;
      // for (int j = 0; j < channels_out; j++)
      // {
      //   if (crx[i][j]>crx_max) crx_max = crx[i][j];
      //   if (crx[i][j]<crx_min) crx_min = crx[i][j];
      // }
      // cout << "crx_max: " << crx_max << endl;
      // cout << "crx_min: " << crx_min << endl;
      ...
      //   if (crf[i][j]<crf_min) crf_min = crf[i][j];
      // }
      // cout << "After crf_max: " << crf_max << endl;
      // cout << "After crf_min: " << crf_min << endl;

      // int crf_max = 0;
      // int crf_min = 0;
      // for (int j = 0; j < channels_out; j++)
      // {
      //   if ((crf[i][j]+crx[i][j])>crf_max) crf_max = crf[i][j]+crx[i][j];
      //   if ((crf[i][j]+crx[i][j])<crf_min) crf_min = crf[i][j]+crx[i][j];
      // }
      // cout << "(crf[i][j]+crx[i][j])max: " << crf_max << endl;
      // cout << "(crf[i][j]+crx[i][j])min: " << crf_min << endl;
