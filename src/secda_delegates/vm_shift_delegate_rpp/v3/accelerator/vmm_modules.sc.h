...
int VMM_UNIT::Quantised_Multiplier_approximate(int x, sc_uint<5> multShift, sc_uint<5> shift)
{
  // int aa = shift;
  // int bb = multShift;
  sc_uint<6> nshift1 = (shift+1);
  sc_uint<6> nshift2 = (shift+multShift);

  sc_int<32> result1 = x >> nshift1;
  sc_int<32> result2 = x >> nshift2;
  sc_int<32> result_32 = result1+result2;
...
int VMM_UNIT::Quantised_Multiplier_ruy_reference(int x, int qm, sc_int<8> shift) {
  ...
  int aa = shift;
  int bb = qm;
  sc_int<8> nshift = (shift+1);
  sc_int<32> result1 = x >> nshift;
  sc_int<32> result2 = x >> (shift+qm);
  sc_int<32> result_32 = result1+result2;
  return result_32;
}
...
void VMM_UNIT::PPU_RPP(int *x, sc_int<8> *pcrf, sc_int<8> *pex, sc_int<32> *g,
                   sc_int<8> *r) {
  int accum[16];
...
#pragma HLS pipeline II = 1
      ...
      sc_uint<5> firstShift = (int)pex[j];
      sc_uint<5> secondShift = (int)pcrf[j];
      int ret_accum1 = Quantised_Multiplier_approximate(accum1, secondShift, firstShift);
#else
      sc_uint<5> firstShift = (int)pex[j];
      sc_uint<5> secondShift = (int)pcrf[j];
      int ret_accum1 = Quantised_Multiplier_approximate(accum1, secondShift, firstShift);
...
//       }
      ACC_DTYPE<32> crf = post_fifo.read();
      ACC_DTYPE<32> ex = post_fifo.read();

      pcrf[0] = crf.range(7, 0);
      pcrf[1] = crf.range(15, 8);
      pcrf[2] = crf.range(23, 16);
      pcrf[3] = crf.range(31, 24);
