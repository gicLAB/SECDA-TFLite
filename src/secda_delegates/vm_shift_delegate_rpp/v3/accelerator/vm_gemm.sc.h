...
sc_int<32> VMM_UNIT::mul_s8(sc_int<8> a, sc_int<8> b) {
  sc_int<32> c;
#ifdef HEIDL_MULT_CORE
...
#ifdef MULT_HEIDL_LOOKUP_TABLE
  c = scdm841_lookupTable[(a+128)][(b+128)];
  // c = scdm842_lookupTable[(a+128)][(b+128)];
#else
  c = a * b;
#endif
...
#pragma HLS pipeline II = 1
    ...
    for (int i = 0; i < 4; i++) {
#pragma HLS unroll
      in_a[i + 0] = wgt_read[i].range(7, 0);
      in_a[i + 4] = wgt_read[i].range(15, 8);
      in_b[i + 0] = wgt_read[i].range(23, 16);
      in_b[i + 4] = wgt_read[i].range(31, 24);

      we_a[i + 0] = inp_read[i].range(7, 0);
      we_a[i + 4] = inp_read[i].range(15, 8);
      we_b[i + 0] = inp_read[i].range(23, 16);
      we_b[i + 4] = inp_read[i].range(31, 24);
    }
    for (int i = 0; i < 4; i++) {
#pragma HLS unroll
