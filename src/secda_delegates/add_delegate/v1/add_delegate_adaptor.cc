...
TfLiteDelegate *CreateAddDelegateFromOptions(char **options_keys,
                                              char **options_values,
                                              size_t num_options) {
  AddDelegateOptions options = TfLiteAddDelegateOptionsDefault();

  // Parse key-values options to AddDelegateOptions by mimicking them as
  // command-line flags.
  std::vector<const char *> argv;
  argv.reserve(num_options + 1);
  constexpr char kAddDelegateParsing[] = "add_delegate_parsing";
  argv.push_back(kAddDelegateParsing);
  ...
  return TfLiteAddDelegateCreate(&options);
}
...
tflite_plugin_create_delegate(char **options_keys, char **options_values,
  ...
  return tflite::tools::CreateAddDelegateFromOptions(
      options_keys, options_values, num_options);
}

TFL_CAPI_EXPORT void tflite_plugin_destroy_delegate(TfLiteDelegate *delegate) {
  TfLiteAddDelegateDelete(delegate);
}
