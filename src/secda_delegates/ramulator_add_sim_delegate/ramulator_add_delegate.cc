...
#include "accelerator/driver/ramulator_add_driver.h"
#include "tensorflow/lite/delegates/utils/secda_delegates/ramulator_add_sim_delegate/ramulator_add_delegate.h"
#include "tensorflow/lite/delegates/utils/secda_delegates/ramulator_add_sim_delegate/util.h"
...
// struct rconnect *rconn;
struct sysC_sigs *scs;
struct Profile *profile;

namespace tflite {
namespace ramulator_add_test {

// Ramulator_add delegate kernel.
class Ramulator_addDelegateKernel : public SimpleDelegateKernelInterface {
public:
  explicit Ramulator_addDelegateKernel(
      const Ramulator_addDelegateOptions &options)
      : options_(options) {}
  ...
  TfLiteStatus Init(TfLiteContext *context,
    ...
    if (!dparams.init) {
      ...
      static struct stream_dma _sdma(0, 0, 0x16000000, 8096, 0, 0x16800000,
                                     8096);
      // static struct rconnect _rconn;
      ...
      // rconn = &_rconn;
      profile = &_profile;
      sdma->dmad->rm.use_sim = options_.use_simmode;
      dparams.init = true;
    }
  ...
  // computation to the ramulator_add_driver For more info look into
  // "tensorflow/lite/kernels/add.cc" for the default implementation for Add
  // Nodes
  TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      // Create ramulator memory blocks for these tensors
      const TfLiteTensor *input1;
      const TfLiteTensor *input2;
      TfLiteTensor *output;
      ...
      sdma->dmad->rm.use_sim = options_.use_simmode;
      sdma->dmad->rm.layer = dparams.layer;
      
      
      drv.clear_traces();
      tflite_ramulator_addsim::Entry(drv);
      dparams.layer++;
      dparams.delegated_nodes--;
    }

    if (dparams.delegated_nodes == 0) {
      profile->saveCSVRecords(".data/ramulator_add_sim");
    }
    return kTfLiteOk;
  }
...
private:
  const Ramulator_addDelegateOptions options_;
};

// Ramulator_addDelegate implements the interface of SimpleDelegateInterface.
// This holds the Delegate capabilities.
class Ramulator_addDelegate : public SimpleDelegateInterface {
public:
  explicit Ramulator_addDelegate(const Ramulator_addDelegateOptions &options)
      : options_(options) {}
  ...
  const char *Name() const override {
    static constexpr char kName[] = "Ramulator_addDelegate";
    return kName;
  }

  std::unique_ptr<SimpleDelegateKernelInterface>
  CreateDelegateKernelInterface() override {
    return std::make_unique<Ramulator_addDelegateKernel>(options_);
  }
...
Ramulator_addDelegateOptions TfLiteRamulator_addDelegateOptionsDefault() {
  Ramulator_addDelegateOptions options = {0};
  // Just assign an invalid builtin code so that this ramulator_add test
  // d elegate will not support any node by default.
  options.allowed_builtin_code = -1;
  return options;
}

// Creates a new delegate instance that need to be destroyed with
// `TfLiteRamulator_addDelegateDelete` when delegate is no longer used by
// TFLite. When `options` is set to `nullptr`, the above default values are
// used:
TfLiteDelegate *
TfLiteRamulator_addDelegateCreate(const Ramulator_addDelegateOptions *options) {
  std::unique_ptr<tflite::ramulator_add_test::Ramulator_addDelegate>
      ramulator_add(new tflite::ramulator_add_test::Ramulator_addDelegate(
          options ? *options : TfLiteRamulator_addDelegateOptionsDefault()));
  return tflite::TfLiteDelegateFactory::CreateSimpleDelegate(
      std::move(ramulator_add));
}
