...
TfLiteDelegate* CreateRamulator_addDelegateFromOptions(char** options_keys,
                                               char** options_values,
                                               size_t num_options) {
  Ramulator_addDelegateOptions options = TfLiteRamulator_addDelegateOptionsDefault();

  // Parse key-values options to Ramulator_addDelegateOptions by mimicking them as
  // command-line flags.
  std::vector<const char*> argv;
  argv.reserve(num_options + 1);
  constexpr char kRamulator_addDelegateParsing[] = "ramulator_add_delegate_parsing";
  argv.push_back(kRamulator_addDelegateParsing);
  ...
  return TfLiteRamulator_addDelegateCreate(&options);
}
...
TFL_CAPI_EXPORT TfLiteDelegate* tflite_plugin_create_delegate(
  ...
  return tflite::tools::CreateRamulator_addDelegateFromOptions(
      options_keys, options_values, num_options);
}

TFL_CAPI_EXPORT void tflite_plugin_destroy_delegate(TfLiteDelegate* delegate) {
  TfLiteRamulator_addDelegateDelete(delegate);
}
