...
void ACCNAME::ReadRows() {
    wait();
    while(1) {
        while(rowReadReadyS.read() == 0) { // Synchronisation
            wait();
        }

        rowReadDoneS.write(0);
        wait();
        DWAIT(2);
        ...
        for (int i_row = 0; i_row < pN_rem; i_row++) { // Loop 1.1.1
            ...
            for (int k_row = 0; k_row < pK; k_row += 4) { // Loop 1.1.1.1
                ...
                // cout << "rows[" << i_row << "][" << k_row+0 << "] = " << rows[i_row][k_row+0] << endl;
                DWAIT(3); // TODO: For some reason this random DWAIT is breaking everything ???
            }
            // cout << "i = " << i << endl;
        }
        // cout << "Rows read"<<endl;
        rowReadDoneS.write(1);
