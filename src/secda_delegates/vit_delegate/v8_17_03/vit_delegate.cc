    ...
    for (int i = 0; i < node_count; i++) {
      ...
      if (builtin_code_[i] == kTfLiteBuiltinConv2d) {
        ...
        Prepare_CONV2D_INT8(context, node, i, layers_params[i], opdatas[i],
                            ...
                            wt_sum3[i], wt_sum4[i], biases[i], crf_arr[i],
                            crx_arr[i], preloaded_weights[i],
  ...
  TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) override {
    ...
    auto ensure_weight_cache = [&](int node_index, int rows, int cols,
      ...
      precal_sum_load_padv3_vectorized(
          const_cast<int8_t *>(raw_weights), rows, cols,
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      if (builtin_code_[i] == kTfLiteBuiltinConv2d) { // CONV2D
        ...
        drv.M = filter_rows;
        drv.K = filter_cols;
        // cout << "CONV Layer N: " << drv.N << " M: " << drv.M << " K: " << drv.K
        //      << endl;
        ...
        drv.padded_weights =
            preloaded_weights[i].empty() ? nullptr : preloaded_weights[i].data();
        ...
        // cout << "crf: " << data->output_multiplier << " crx: " << data->output_shift
        //      << " output_offset: " << output_offset << " rhs_offset: " << input_offset << " lhs_offset: " << filter_offset << endl;
