...
void block_add(acc_container &drv) {
  ...
  if (drv.layer_t != 0) mode = (dense_bytes <= mobile_bytes) ? MODE_DENSE : MODE_MOBILE;
#endif
  ...
  if (mode == 0) { 
    for (int n = 0; n < drv.pN; n += HW_ROWS) {
      ...
      for (int m = 0; m < drv.pM; m += HW_DENSE_W) {
        ...
        // --- 4-DMA OUTPUT RECEIVE (DENSE) ---
        // Post receives BEFORE waiting for sends to avoid AXI stream deadlock
        prf_start(9);
        int recv_size = (HW_ROWS * HW_COLS) / 4; 
        
        drv.mdma->dmas[0].dma_start_recv(recv_size);
        drv.mdma->dmas[1].dma_start_recv(recv_size);
        drv.mdma->dmas[2].dma_start_recv(recv_size);
        drv.mdma->dmas[3].dma_start_recv(recv_size);
        
        // Wait for sends and receives concurrently
        drv.mdma->multi_dma_wait_send();
        drv.mdma->multi_dma_wait_recv();

        int8_t *out_val[4];
        out_val[0] = (int8_t*)drv.mdma->dmas[0].dma_get_outbuffer();
        out_val[1] = (int8_t*)drv.mdma->dmas[1].dma_get_outbuffer();
        out_val[2] = (int8_t*)drv.mdma->dmas[2].dma_get_outbuffer();
        out_val[3] = (int8_t*)drv.mdma->dmas[3].dma_get_outbuffer();
            ...
            for (int c = 0; c < HW_CORES; c++) {
                ...
                if (n_sub > 0) {
                    for (int i = 0; i < n_sub; i++) {
                        ...
                        memcpy(drv.padded_output + base, out_val[c] + (i * HW_COLS), m_rem);
                    }
                }
            }
            prf_end(9, drv.a_t->out_copy);
