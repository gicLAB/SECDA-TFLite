...
unsigned int dma_addrs[1] = {dma_addr0}; // TODO: Found in acc_config"
...
#ifdef SYSC
static struct multi_dma mdma(1, dma_addrs, dma_addrs_in, dma_addrs_out,
                             16192); // was 563840, made it stupid big
ACCNAME *acc;
...
public:
  ...
  TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      // re-quantization)
      // prf_start(3);
      // precal_sum_load_pad(input->data.int8, N, K, padded_input,
      //                                  in_sum, 16, unroll);
      ...
      //       int mul = in * wt;
      //       // if(m==4)  cout << "in = " << in << " wt = " << wt << " mul = "
      //       <<
      //       // mul << endl;
      //       sum += in * wt;
      //     }
      //     // if(m==4)  cout << "sum = " << sum << endl;
      //     //  add bias
      //     if (n == 0 && m == 0) {
      //       int k = 0;
      //     }
      //     // TODO:  Issue with bias
      //     if (isBias) sum += biases[i][m];
      //     sum += (wt_sum[m] * (-rhs_offset)) + (in_sum[n] * (-lhs_offset));
      //     // if(m==4)  cout << "sum = " << sum << endl;
      ...
      // acc_container is used to wrap all the parameters the
      // fc_driver / accelerator needs from the delegate

      // Make padded things be able to get sent to acc
      // TODO: I am using padded input here as I think that is waht precal
      // sum load deals with, if I am wrong it could be on input and filter
      // instead

      // cout << "Padded input: " << padded_input << endl;
      struct acc_container drv;
      // TODO: Need to implement size
      // drv.scs = scs;
      drv.profile = &profile;
      drv.acc = acc;
      drv.mdma = &mdma;
      ...
      // cout << "output data shape = " << output_data.shape
      //  Read the output data here
      //  Loop over the array and print its values

      // cout << "N = " << N << " M = " << M << endl;
      // for (int i = 0; i < N; ++i) {
      //   for (int j = 0; j < M; ++j) {
      //     std::cout << static_cast<int>(output_data[i * M + j]) << " ";
      //   }
      //   std::cout << std::endl;
      // }
      // cout << endl;

      saveMatrixCSV("aData/ViT_Tiny_vit_1_0/" + std::to_string(associated_nodes[i]) +
                        ".csv",
                    output_data, N, M);
    ...
    // profile->saveCSVRecords(".data/mha_sim");
    //}
    return kTfLiteOk;
  };
...
void TfLiteVitDelegateDelete(TfLiteDelegate *delegate) {
  ...
  SYSC_ON(profile.saveCSVRecords(".data/vit_sim_v1"));
#ifndef SYSC
  if (!dparams.unmap) {
