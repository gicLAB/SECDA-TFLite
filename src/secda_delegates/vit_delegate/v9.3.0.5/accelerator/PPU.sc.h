...
void ACCNAME::PPU() {
  ...
  while (1) {
    ...
    int cur_ra = ra;
    const bool stripe_output = (layer_t == 0);
    const int total_words = NUM_CORES * pn_block * (pm_block / 4);
    const int ch_words[4] = {
        (total_words + 3) / 4,
        (total_words + 2) / 4,
        (total_words + 1) / 4,
        total_words / 4,
    };
    int ch_counts[4] = {0, 0, 0, 0};
    int word_idx = 0;
    DWAIT(1);
...
#pragma HLS array_partition variable = local_sm complete dim = 0
          ...
          // --- FIX: On-the-fly Calculation ---
          // We read the small components individually.
          // Since they are small/partitioned arrays, this is very fast.
          // wait()
...
#pragma HLS UNROLL
            ...
            value3 = Quantised_Multiplier_Conv(local_res[2], local_crf[2],
                                               local_crx[2]);            int cur_crf = crf;

            value4 = Quantised_Multiplier_Conv(local_res[3], local_crf[3],
                                               local_crx[3]);
#else
            value1 = Quantised_Multiplier_Conv(local_res[0], local_crf[0],
                                               local_pl[0], local_pr[0],
                                               local_msk[0], local_sm[0]);
            value2 = Quantised_Multiplier_Conv(local_res[1], local_crf[1],
                                               local_pl[1], local_pr[1],
                                               local_msk[1], local_sm[1]);
            value3 = Quantised_Multiplier_Conv(local_res[2], local_crf[2],
                                               local_pl[2], local_pr[2],
                                               local_msk[2], local_sm[2]);
            value4 = Quantised_Multiplier_Conv(local_res[3], local_crf[3],
                                               local_pl[3], local_pr[3],
                                               local_msk[3], local_sm[3]);
#endif
            // wait();
            ...
            // wait();
            value1 =
                Quantised_Multiplier_FC(res[core][i][j + 0], cur_crf, cur_crx);
            value2 =
                Quantised_Multiplier_FC(res[core][i][j + 1], cur_crf, cur_crx);
            value3 =
                Quantised_Multiplier_FC(res[core][i][j + 2], cur_crf, cur_crx);
            value4 =
                Quantised_Multiplier_FC(res[core][i][j + 3], cur_crf, cur_crx);
            // wait();
          }
          // wait();
          ...
          //   wait(); // Error is currently happening in 134 and 141
          ...
          if (stripe_output) {
            const int out_ch = word_idx & 0x3;
            ch_counts[out_ch]++;
            d_out.tlast = (ch_counts[out_ch] == ch_words[out_ch]);
            ...
            d_out.tlast =
                (core == NUM_CORES - 1 && i == pn_block - 1 && j == pm_block - 4);
            dout1.write(d_out);
          }
