  ...
  TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      if (builtin_code_[i] == kTfLiteBuiltinConv2d) { // CONV2D
        ...
        if (data->need_im2col && im2col != nullptr) {
          RuntimeShape im2col_shape =
              RuntimeShape(im2col->dims->size, im2col->dims->data);
          const size_t required_im2col_bytes =
              static_cast<size_t>(im2col_shape.FlatSize()) * sizeof(int8_t);
          if (temp_im2col[i].size() < required_im2col_bytes) {
            temp_im2col[i].resize(required_im2col_bytes);
          }
        }
