...
bool Prepare_CONV2D_INT8(TfLiteContext *context, TfLiteNode *node, int i,
  ...
  const int input_depth_for_im2col = input->dims->data[3];
  ...
  const size_t im2col_bytes = static_cast<size_t>(batches) * out_height *
                              out_width * input_depth_for_im2col * filter_height *
  ...
  int padded_channels = ((channels_out + 16 - 1) / 16) * 16;
  ...
  // MOD: Pre-compute the offset cross-term to safely resolve asymmetrical quantization gaps
  ...
  // Add the cross-term into the actual hardware-computed bias
  for (int j = 0; j < channels_out; j++) {
    biases[j] += cross_term;
  }
  
  // Important: Resize pads with 'cross_term' rather than '0' so padding elements
  // don't mathematically distort PPU outputs before they are legitimately discarded
  ...
  const int padded_rows = ((filter_rows + 16 - 1) / 16) * 16;
  ...
  precal_sum_load_padv3_vectorized(
      ...
      preloaded_weights.data(), preloaded_wt_sum);
...
bool Prepare_FC_INT8(TfLiteContext *context, TfLiteNode *node, int i,
  ...
  // MOD: Pre-emptively pad quantization parameters to match padded execution layout
  crf.resize(padded_channels, 0);
  crx.resize(padded_channels, 0);
  ...
  // MOD: Compute and resolve missing TFLite cross-term 
  const int32_t input_offset = -input->params.zero_point;
  const int32_t filter_offset = -filter->params.zero_point;
  ...
  const int padded_rows = ((output_depth + 16 - 1) / 16) * 16;
