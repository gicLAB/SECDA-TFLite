...
public:
  ...
  TfLiteStatus Init(TfLiteContext *context,
    ...
    conv_im2col.resize(params->nodes_to_replace->size);
    ...
    for (int i = 0; i < params->nodes_to_replace->size; ++i) {
      ...
      // Store inputs and outputs based on layer type
      ...
      // Store builtin_data and user_data
  ...
  TfLiteStatus Prepare(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      if (builtin_code_[i] == kTfLiteBuiltinFullyConnected) {
        ...
        OpData *data = reinterpret_cast<OpData *>(opdatas[i]);
        ...
        TfLitePaddingValues padding = ComputePaddingHeightWidth(
            ...
            input->dims->data[1], input->dims->data[2], filter->dims->data[1],
            filter->dims->data[2], params->padding, &padding_height,
            &padding_width);
        ...
        const int output_height = ComputeOutSize(
            params->padding, input_height, filter_height, params->stride_height,
            params->dilation_height_factor);
        const int output_width =
            ComputeOutSize(params->padding, input_width, filter_width,
                           params->stride_width, params->dilation_width_factor);
        ...
        // Handle temporary tensor allocation if needed
        int temp_out_id;
        bool req_temp_out = outputs_[i][0] != node->outputs->data[out_tid];
        if (!req_temp_out) out_tid++;
        ...
        if (req_temp_out) {
          node->temporaries->data[temp_out_id] = outputs_[i][0];
          TfLiteIntArray *temp_out_tensor_size =
              TfLiteIntArrayCopy(output_size);
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      if (builtin_code_[i] == kTfLiteBuiltinFullyConnected) {
        ...
        TfLitePaddingValues padding = ComputePaddingHeightWidth(
            ...
            params->padding, &padding_height, &padding_width);

        // Simple CONV2D implementation - can be optimized with accelerator
        // later
          ...
          for (int out_y = 0; out_y < output_height; ++out_y) {
            for (int out_x = 0; out_x < output_width; ++out_x) {
              for (int out_c = 0; out_c < output_channels; ++out_c) {
                  ...
                  for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
                    for (int in_c = 0; in_c < input_depth; ++in_c) {
                      const int in_y = out_y * params->stride_height +
                                       filter_y - padding.height;
                      const int in_x = out_x * params->stride_width + filter_x -
                                       padding.width;
                      ...
                      if (in_y >= 0 && in_y < input_height && in_x >= 0 &&
                        ...
                        int input_val =
                            input_data[batch * input_height * input_width *
                                       ...
                                       in_x * input_depth + in_c];
                        int filter_val =
                            filter_data[out_c * filter_height * filter_width *
                                            input_depth +
                                        filter_y * filter_width * input_depth +
                                        filter_x * input_depth + in_c];
                        acc +=
                            (input_val + op_params.input_offset) * filter_val;
                      }
                    }
                  }
                }
                ...
                if (bias) {
                  acc += bias->data.i32[out_c];
                }

                // Apply quantization
                acc = Quantised_Multiplier_S(acc, data->output_multiplier,
                                             data->output_shift);
                acc += op_params.output_offset;
                acc = std::max(
                    acc, static_cast<int32_t>(data->output_activation_min));
                acc = std::min(
                    acc, static_cast<int32_t>(data->output_activation_max));

                output_data[batch * output_height * output_width *
                                output_channels +
  ...
  std::vector<std::vector<int8_t>> conv_im2col;
