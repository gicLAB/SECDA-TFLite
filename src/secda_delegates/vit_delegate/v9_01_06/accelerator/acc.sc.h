...
#pragma HLS protocol fixed
  ...
  ACCNAME(sc_module_name name_) : sc_module(name_) {
    ...
    SC_CTHREAD(ReadInp, clock.pos());
    reset_signal_is(reset, true);

    SC_CTHREAD(ReadWgt, clock.pos());
    reset_signal_is(reset, true);

    SC_CTHREAD(ReadBias, clock.pos());
    reset_signal_is(reset, true);

    SC_CTHREAD(PE, clock.pos());
    reset_signal_is(reset, true);

    SC_CTHREAD(PPU, clock.pos());
    reset_signal_is(reset, true);

    // HWC monitoring thread
...
HWC_PragGroup(ReadInp);
HWC_PragGroup(ReadWgt);
HWC_PragGroup(ReadBias);
HWC_PragGroup(PE);
HWC_PragGroup(PPU);
