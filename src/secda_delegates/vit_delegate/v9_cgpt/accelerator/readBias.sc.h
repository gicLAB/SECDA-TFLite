...
void ACCNAME::ReadBias() {
  ...
  while (1) {
    ...
    int load_static = din1->read().data;
    ...
    // 4. Input Sums (Optimized: Removed 'prec' calculation)
    // We only calculate in_sum here. The final addition happens in PPU.
    if (current_mode == MODE_DENSE) {
      for (int a = 0; a < pn_block; a++) {
        ...
        for (int core = 0; core < NUM_CORES; core++) {
          in_sum[core][a] = i_sum;
          // REMOVED: prec[...] calculation loop to save memory ports
        }
      }
    } else {
      for (int core = 0; core < NUM_CORES; core++) {
