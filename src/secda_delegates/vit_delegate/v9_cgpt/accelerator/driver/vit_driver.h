...
void block_add(acc_container &drv) {
  ...
  const int CORE_BLOCK_SIZE = HW_ROWS * HW_COLS; // 4096 bytes per core block
    ...
    const int64_t dense_wgt_bytes = static_cast<int64_t>(drv.pN) * drv.pK *
                                                                    ((drv.pM + HW_COLS - 1) / HW_COLS);
    const int64_t dense_inp_bytes = static_cast<int64_t>(drv.pN) * drv.pK *
                                                                    ((drv.pM + HW_DENSE_W - 1) / HW_DENSE_W);
    const int64_t mobile_wgt_bytes = static_cast<int64_t>(drv.pM) * drv.pK;
    const int64_t mobile_inp_bytes = static_cast<int64_t>(drv.pN) * drv.pK *
                                                                     ((drv.pM + HW_COLS - 1) / HW_COLS);

    // Weight traffic dominates DMA2; bias mode selection toward lower weight bytes.
    const int64_t dense_cost = (dense_wgt_bytes * 3) + dense_inp_bytes;
    const int64_t mobile_cost = (mobile_wgt_bytes * 3) + mobile_inp_bytes;
    int mode = (dense_cost <= mobile_cost) ? MODE_DENSE : MODE_MOBILE;
  ...
  if (mode == 0) { 
    for (int n = 0; n < drv.pN; n += HW_ROWS) {
      ...
      // 1. Inputs (Shared)
            ...
            if (n_rem == HW_ROWS) {
                memcpy(input_buf + input_len, drv.padded_input + n * drv.pK, total_bytes);
            } else {
      ...
      prf_end(2, drv.a_t->inp_copy);
            ...
            for (int m = 0; m < drv.pM; m += HW_DENSE_W) {
                const bool load_static_params = (n == 0);
                const int rem_cols = std::max(0, drv.pM - m);
                const int active_cores =
                        std::min(HW_CORES, (rem_cols + HW_COLS - 1) / HW_COLS);
        ...
        // 2. Weights (Unique)
            ...
            if (m_rem > 0) {
                if (m_rem == HW_COLS) {
                    memcpy(weight_buf + weight_len,
                           drv.padded_weights + cur_m * drv.pK, w_bytes);
                } else {
                    int w_valid = drv.pK * m_rem;
                    memcpy(weight_buf + weight_len,
                           drv.padded_weights + cur_m * drv.pK, w_valid);
        ...
        param_buf[param_len++] = load_static_params ? 1 : 0;

        // 3. Params
        // prf_start(1); // bias_copy
        if (load_static_params && drv.layer_t == 1) {
            for (int c = 0; c < HW_CORES; c++) {
                 int cur_m = m + (c * HW_COLS);
                 int m_rem = (cur_m < drv.pM) ? std::min(HW_COLS, drv.pM - cur_m) : 0;
                 if (m_rem > 0) {
                     if (m_rem == HW_COLS) {
                         memcpy(param_buf + param_len, drv.crf_a.data() + cur_m,
                                HW_COLS * 4);
                     } else {
                         memcpy(param_buf + param_len, drv.crf_a.data() + cur_m,
                                m_rem * 4);
                         memset(param_buf + param_len + m_rem, 0,
                                (HW_COLS - m_rem) * 4);
                     }
                 } else {
                 ...
                 if (m_rem > 0) {
                      if (m_rem == HW_COLS) {
                          memcpy(bias_buf + bias_len, drv.crx_a.data() + cur_m,
                                 HW_COLS);
                      } else {
                          memcpy(bias_buf + bias_len, drv.crx_a.data() + cur_m,
                                 m_rem);
        ...
        if (load_static_params && drv.is_bias) {
             for (int c = 0; c < HW_CORES; c++) {
                 int cur_m = m + (c * HW_COLS);
                 int m_rem = (cur_m < drv.pM) ? std::min(HW_COLS, drv.pM - cur_m) : 0;
                 if (m_rem > 0) {
                     if (m_rem == HW_COLS) {
                         memcpy(bias_buf + bias_len, drv.bias + cur_m,
                                HW_COLS * 4);
                     } else {
                         memcpy(bias_buf + bias_len, drv.bias + cur_m,
                                m_rem * 4);
                         memset(bias_buf + bias_len + m_rem, 0,
                                (HW_COLS - m_rem) * 4);
                     }
                 } else {
                ...
                if (load_static_params) {
                    for (int c = 0; c < HW_CORES; c++) {
             int cur_m = m + (c * HW_COLS);
             int m_rem = (cur_m < drv.pM) ? std::min(HW_COLS, drv.pM - cur_m) : 0;
             if (m_rem > 0) {
                 if (m_rem == HW_COLS) {
                     memcpy(param_buf + param_len, drv.wt_sum + cur_m,
                            HW_COLS * 4);
                 } else {
                     memcpy(param_buf + param_len, drv.wt_sum + cur_m,
                            m_rem * 4);
    ...
    for (int m = 0; m < drv.pM; m += HW_COLS) {
        ...
        int w_bytes = drv.pK * HW_COLS;
         ...
         if (m_rem == HW_COLS) {
             ...
             memcpy(weight_buf + weight_len, drv.padded_weights + m * drv.pK,
                 drv.pK * m_rem);
             memset((int8_t *)weight_buf + (weight_len * 4) + (drv.pK * m_rem),
                 0, drv.pK * (HW_COLS - m_rem));
         }
        weight_len += w_bytes / 4;
        ...
        for (int n = 0; n < drv.pN; n += HW_MOBILE_IN) {
                ...
                if (n_sub > 0) {
                    if (n_sub == HW_ROWS) {
                        memcpy(input_buf + input_len,
                               drv.padded_input + cur_n * drv.pK, i_bytes);
                    } else {
                        memcpy(input_buf + input_len,
                               drv.padded_input + cur_n * drv.pK,
                               drv.pK * n_sub);
                        memset((int8_t *)input_buf + (input_len * 4) +
                                   (drv.pK * n_sub),
                               0, drv.pK * (HW_ROWS - n_sub));
                    }
                } else {
            ...
            if (load_static_params) {
                if (m_rem == HW_COLS) {
                    memcpy(param_buf + param_len, drv.wt_sum + m, HW_COLS * 4);
                } else {
            ...
            if (load_static_params && drv.is_bias) {
                if (m_rem == HW_COLS) {
                    memcpy(bias_buf + bias_len, drv.bias + m, HW_COLS * 4);
                } else {
            ...
            for (int c = 0; c < HW_CORES; c++) {
                 ...
                 int n_sub = (cur_n < drv.pN) ? std::min(HW_ROWS, drv.pN - cur_n) : 0;
                   if (n_sub == HW_ROWS) {
                    memcpy(bias_buf + bias_len, drv.in_sum + cur_n,
                        HW_ROWS * 4);
                   } else {
                    memcpy(bias_buf + bias_len, drv.in_sum + cur_n,
                        n_sub * 4);
                    memset(bias_buf + bias_len + n_sub, 0,
                        (HW_ROWS - n_sub) * 4);
                   }
                 bias_len += HW_ROWS;
            }
