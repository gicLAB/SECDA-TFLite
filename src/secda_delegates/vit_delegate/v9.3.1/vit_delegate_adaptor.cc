...
TfLiteDelegate *CreateVitDelegateFromOptions(char **options_keys,
                                              char **options_values,
                                              size_t num_options) {
  VitDelegateOptions options = TfLiteVitDelegateOptionsDefault();

  // Parse key-values options to VitDelegateOptions by mimicking them as
  // command-line flags.
  std::vector<const char *> argv;
  argv.reserve(num_options + 1);
  constexpr char kVitDelegateParsing[] = "vit_delegate_parsing";
  argv.push_back(kVitDelegateParsing);
  ...
  return TfLiteVitDelegateCreate(&options);
}
...
tflite_plugin_create_delegate(char **options_keys, char **options_values,
  ...
  return tflite::tools::CreateVitDelegateFromOptions(
      options_keys, options_values, num_options);
}

TFL_CAPI_EXPORT void tflite_plugin_destroy_delegate(TfLiteDelegate *delegate) {
  TfLiteVitDelegateDelete(delegate);
}
