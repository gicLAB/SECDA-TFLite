  ...
  if (mode == 0) { 
    for (int n = 0; n < drv.pN; n += HW_ROWS) {
      ...
      int n_rem = std::min(HW_ROWS, drv.pN - n);
            const bool full_n_dense = (n_rem == HW_ROWS);
      ...
      for (int m = 0; m < drv.pM; m += HW_DENSE_W) {
        prf_start(5);
                const bool full_m_dense = (m + HW_DENSE_W <= drv.pM);
        // 2. Weights (Unique)
                if (full_m_dense) {
                        const int w_total_bytes = drv.pK * HW_DENSE_W;
                        memcpy(weight_buf + weight_len, drv.padded_weights + m * drv.pK,
                                     w_total_bytes);
                        weight_len += w_total_bytes / 4;
                } else {
        ...
        if (drv.layer_t == 1) {
            if (full_m_dense) {
                memcpy(param_buf + param_len, drv.crf_a.data() + m,
                       HW_DENSE_W * 4);
                param_len += HW_DENSE_W;

                memcpy((int8_t*)(bias_buf + bias_len), drv.crx_a.data() + m,
                       HW_DENSE_W);
                bias_len += HW_CORES * ((HW_COLS + 3) / 4);
            } else {
              for (int c = 0; c < HW_CORES; c++) {
                   int cur_m = m + (c * HW_COLS);
                   int m_rem = (cur_m < drv.pM) ? std::min(HW_COLS, drv.pM - cur_m) : 0;
        ...
        if (drv.is_bias) {
             if (full_m_dense) {
                 memcpy(bias_buf + bias_len, drv.bias + m, HW_DENSE_W * 4);
                 bias_len += HW_DENSE_W;
             } else {
               for (int c = 0; c < HW_CORES; c++) {
                   int cur_m = m + (c * HW_COLS);
        ...
        if (full_m_dense) {
            memcpy(param_buf + param_len, drv.wt_sum + m, HW_DENSE_W * 4);
            param_len += HW_DENSE_W;
        } else {
          for (int c = 0; c < HW_CORES; c++) {
               int cur_m = m + (c * HW_COLS);
        ...
        if (full_n_dense) {
          memcpy(bias_buf + bias_len, drv.in_sum + n, HW_ROWS * 4);
        } else {
        ...
        // We need to reconstruct linear rows.
                ...
                if (full_m_dense) {
                    for (int i = 0; i < n_rem; i++) {
                        int row_base = (n + i) * drv.pM + m;
                        int src_base = i * HW_COLS;
                        for (int c = 0; c < HW_CORES; c++) {
                            memcpy(drv.padded_output + row_base + (c * HW_COLS),
                                         output_val + (c * HW_ROWS * HW_COLS) + src_base,
                                         HW_COLS);
                        }
                    }
         } else {
  ...
  // =========================================================================
    ...
    for (int m = 0; m < drv.pM; m += HW_COLS) {
        ...
        for (int n = 0; n < drv.pN; n += HW_MOBILE_IN) {
            prf_start(2);
            const bool full_n_mobile = (n + HW_MOBILE_IN <= drv.pN);

            // 2. Inputs
            if (full_n_mobile) {
                const int i_total_bytes = drv.pK * HW_MOBILE_IN;
                memcpy(input_buf + input_len, drv.padded_input + n * drv.pK,
                       i_total_bytes);
                input_len += i_total_bytes / 4;
            } else {
              for (int c = 0; c < HW_CORES; c++) {
            ...
            if (drv.is_bias) {
              ...
              if (full_n_mobile) {
                 memcpy(bias_buf + bias_len, drv.in_sum + n, HW_MOBILE_IN * 4);
                 bias_len += HW_MOBILE_IN;
              } else {
                for (int c = 0; c < HW_CORES; c++) {
                    int cur_n = n + (c * HW_ROWS);
                    int n_sub = (cur_n < drv.pN) ? std::min(HW_ROWS, drv.pN - cur_n) : 0;
