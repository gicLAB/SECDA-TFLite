...
public:
  ...
  TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      /*
      // Open the dump file in append mode for all layers
      std::ofstream dump_file("vit_delegate_layers_dump.txt", std::ios::app);
      dump_file << "LAYER " << (i + 1) << std::endl;

      // Dump input
      dump_file << "INP: [";
      for (int n = 0; n < N; ++n) {
        for (int k = 0; k < K; ++k) {
          dump_file << static_cast<int>(input_data[n * K + k]);
          if (k != K - 1) dump_file << " ";
        }
        if (n != N - 1) dump_file << "; ";
      }
      dump_file << "]" << std::endl << std::endl; // Blank line between INP and WGT

      // Dump weights
      dump_file << "WGT: [";
      for (int m = 0; m < M; ++m) {
        for (int k = 0; k < K; ++k) {
          dump_file << static_cast<int>(filter_data[m * K + k]);
          if (k != K - 1) dump_file << " ";
        }
        if (m != M - 1) dump_file << "; ";
      }
