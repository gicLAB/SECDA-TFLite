#include "tensorflow/lite/delegates/utils/secda_delegates/vit_delegate/v8/vit_delegate.h"
#include <fstream>
#include <iostream>
#include <algorithm>
#include <cstring>
...
constexpr int kDriverHwRows = pn_block;
constexpr int kDriverHwCols = pm_block;
constexpr int kDriverHwCores = NUM_CORES;
constexpr int kDriverHwDenseW = kDriverHwCols * kDriverHwCores;

int CeilDiv(int value, int divisor) { return (value + divisor - 1) / divisor; }

void AppendPaddedIntBlock(std::vector<int> &dst, const int *src, int valid_count,
                          int padded_count) {
  size_t start = dst.size();
  dst.resize(start + padded_count, 0);
  if (src != nullptr && valid_count > 0) {
    memcpy(dst.data() + start, src, valid_count * sizeof(int));
  }
}

void AppendPaddedByteBlock(std::vector<int> &dst, const int8_t *src,
                           int valid_bytes, int padded_bytes) {
  const int padded_words = (padded_bytes + 3) / 4;
  size_t start = dst.size();
  dst.resize(start + padded_words, 0);
  if (src != nullptr && valid_bytes > 0) {
    memcpy(reinterpret_cast<int8_t *>(dst.data() + start), src, valid_bytes);
  }
}

prepacked_node_dma BuildPrepackedNodeDmaPlan(
    int layer_t, int pN, int pM, int pK, int rhs_offset, int lhs_offset,
    int is_bias, const std::vector<int8_t> &padded_weights,
    const std::vector<int> &wt_sum, const std::vector<int> &bias,
    const std::vector<int> &crf_a, const std::vector<int8_t> &crx_a) {
  prepacked_node_dma plan;
  plan.layer_t = layer_t;
  plan.mode = vit_sim::SelectDriverMode(layer_t, pN, pM, pK, is_bias);
  plan.pM = pM;
  plan.pK = pK;
  plan.is_bias = is_bias;
  plan.rhs_offset = rhs_offset;
  plan.lhs_offset = lhs_offset;

  if (pM <= 0 || pK <= 0 || padded_weights.empty() ||
      wt_sum.size() < static_cast<size_t>(pM)) {
    return plan;
  }

  const int8_t *weight_base = padded_weights.data();
  if (plan.mode == 0) {
    const int tile_count = CeilDiv(pM, kDriverHwDenseW);
    const int weight_words_per_tile = (pK * kDriverHwDenseW) / 4;
    plan.tiles.resize(tile_count);

    for (int tile_idx = 0; tile_idx < tile_count; ++tile_idx) {
      const int m = tile_idx * kDriverHwDenseW;
      prepacked_m_tile_dma &tile = plan.tiles[tile_idx];
      tile.weight_words.assign(weight_words_per_tile, 0);
      int8_t *weight_dst = reinterpret_cast<int8_t *>(tile.weight_words.data());

      for (int c = 0; c < kDriverHwCores; ++c) {
        const int cur_m = m + (c * kDriverHwCols);
        const int m_rem =
            (cur_m < pM) ? std::min(kDriverHwCols, pM - cur_m) : 0;
        int8_t *core_dst = weight_dst + (c * pK * kDriverHwCols);
        if (m_rem > 0) {
          memcpy(core_dst, weight_base + (cur_m * pK), pK * m_rem);
        }
      }
      ...
      if (layer_t == 1) {
        for (int c = 0; c < kDriverHwCores; ++c) {
          ...
          const int m_rem =
              (cur_m < pM) ? std::min(kDriverHwCols, pM - cur_m) : 0;
          const int *crf_ptr =
              (m_rem > 0 && crf_a.size() >= static_cast<size_t>(cur_m + m_rem))
                  ? crf_a.data() + cur_m
                  : nullptr;
          AppendPaddedIntBlock(tile.param_words, crf_ptr, m_rem, kDriverHwCols);
        }
      }
      ...
      for (int c = 0; c < kDriverHwCores; ++c) {
        ...
        const int m_rem =
            (cur_m < pM) ? std::min(kDriverHwCols, pM - cur_m) : 0;
        const int *wt_ptr =
            (m_rem > 0 && wt_sum.size() >= static_cast<size_t>(cur_m + m_rem))
                ? wt_sum.data() + cur_m
                : nullptr;
        AppendPaddedIntBlock(tile.param_words, wt_ptr, m_rem, kDriverHwCols);
      }

      tile.bias_prefix_words.push_back(rhs_offset);
      tile.bias_prefix_words.push_back(lhs_offset);
      ...
      if (layer_t == 1) {
        for (int c = 0; c < kDriverHwCores; ++c) {
          ...
          const int m_rem =
              (cur_m < pM) ? std::min(kDriverHwCols, pM - cur_m) : 0;
          const int8_t *crx_ptr =
              (m_rem > 0 && crx_a.size() >= static_cast<size_t>(cur_m + m_rem))
                  ? crx_a.data() + cur_m
                  : nullptr;
          AppendPaddedByteBlock(tile.bias_prefix_words, crx_ptr, m_rem,
                                kDriverHwCols);
        }
      }
      ...
      if (is_bias) {
        for (int c = 0; c < kDriverHwCores; ++c) {
          ...
          const int m_rem =
              (cur_m < pM) ? std::min(kDriverHwCols, pM - cur_m) : 0;
          const int *bias_ptr =
              (m_rem > 0 && bias.size() >= static_cast<size_t>(cur_m + m_rem))
                  ? bias.data() + cur_m
                  : nullptr;
          AppendPaddedIntBlock(tile.bias_prefix_words, bias_ptr, m_rem,
                               kDriverHwCols);
        }
      }
    }
  } else {
    const int tile_count = CeilDiv(pM, kDriverHwCols);
    const int weight_words_per_tile = (pK * kDriverHwCols) / 4;
    plan.tiles.resize(tile_count);

    for (int tile_idx = 0; tile_idx < tile_count; ++tile_idx) {
      const int m = tile_idx * kDriverHwCols;
      const int m_rem = std::min(kDriverHwCols, pM - m);
      prepacked_m_tile_dma &tile = plan.tiles[tile_idx];
      tile.weight_words.assign(weight_words_per_tile, 0);
      if (m_rem > 0) {
        memcpy(reinterpret_cast<int8_t *>(tile.weight_words.data()),
               weight_base + (m * pK), pK * m_rem);
      }

      if (layer_t == 1) {
        const int *crf_ptr =
            (m_rem > 0 && crf_a.size() >= static_cast<size_t>(m + m_rem))
                ? crf_a.data() + m
                : nullptr;
        AppendPaddedIntBlock(tile.param_words, crf_ptr, m_rem, kDriverHwCols);
      }
      ...
      const int *wt_ptr =
          (m_rem > 0 && wt_sum.size() >= static_cast<size_t>(m + m_rem))
              ? wt_sum.data() + m
              : nullptr;
      AppendPaddedIntBlock(tile.param_words, wt_ptr, m_rem, kDriverHwCols);
      ...
      if (layer_t == 1) {
        const int8_t *crx_ptr =
            (m_rem > 0 && crx_a.size() >= static_cast<size_t>(m + m_rem))
                ? crx_a.data() + m
                : nullptr;
        AppendPaddedByteBlock(tile.bias_prefix_words, crx_ptr, m_rem,
                              kDriverHwCols);
      ...
      if (is_bias) {
        const int *bias_ptr =
            (m_rem > 0 && bias.size() >= static_cast<size_t>(m + m_rem))
                ? bias.data() + m
                : nullptr;
        AppendPaddedIntBlock(tile.bias_prefix_words, bias_ptr, m_rem,
                             kDriverHwCols);
...
bool CanUsePrepackedNodeDma(const prepacked_node_dma &plan,
                            const acc_container &drv, int mode) {
  return plan.valid && plan.layer_t == drv.layer_t && plan.mode == mode &&
         plan.pM == drv.pM && plan.pK == drv.pK &&
         plan.is_bias == drv.is_bias && plan.rhs_offset == drv.rhs_offset &&
         plan.lhs_offset == drv.lhs_offset && !plan.tiles.empty();
}
...
public:
  ...
  TfLiteStatus Init(TfLiteContext *context,
    ...
    prepacked_dma_plans.resize(params->nodes_to_replace->size);
  ...
  TfLiteStatus Prepare(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      if (builtin_code_[i] == kTfLiteBuiltinConv2d) {
        ...
        const TfLiteTensor *prep_input = nullptr;
        const TfLiteTensor *prep_filter = nullptr;
        GetInputSafe(context, inputs_[i][0], &prep_input);
        GetInputSafe(context, inputs_[i][1], &prep_filter);
        const bool is_bias = (inputs_[i].size() == 3 && inputs_[i][2] >= 0);

        int filter_rows = prep_filter->dims->data[0];
        int filter_cols = 1;
        for (int dim = 1; dim < prep_filter->dims->size; ++dim) {
          filter_cols *= prep_filter->dims->data[dim];
        }

        int gemm_input_cols = 1;
        for (int dim = 0; dim < prep_input->dims->size - 1; ++dim) {
          gemm_input_cols *= prep_input->dims->data[dim];
        }
        Conv2D_Data *prep_data = reinterpret_cast<Conv2D_Data *>(opdatas[i]);
        if (prep_data != nullptr && prep_data->need_im2col &&
            prep_data->im2col_id != kTensorNotAllocated) {
          TfLiteTensor *prep_im2col = &context->tensors[prep_data->im2col_id];
          RuntimeShape prep_im2col_shape(
              prep_im2col->dims->size, prep_im2col->dims->data);
          gemm_input_cols = FlatSizeSkipDim(prep_im2col_shape, 3);
        }

        prepacked_dma_plans[i] = BuildPrepackedNodeDmaPlan(
            /*layer_t=*/1, roundUp(gemm_input_cols, 16),
            roundUp(filter_rows, 16), roundUp(filter_cols, 64),
            -prep_input->params.zero_point, -prep_filter->params.zero_point,
            is_bias, preloaded_weights[i], preloaded_weight_sums[i], biases[i],
            crf_arr[i], crx_arr[i]);

        if (prepacked_dma_plans[i].valid) {
          preloaded_weights[i].clear();
          preloaded_weights[i].shrink_to_fit();
          preloaded_weight_sums[i].clear();
          preloaded_weight_sums[i].shrink_to_fit();
        }
        ...
        TfLiteTensor *prep_output = nullptr;
        GetOutputSafe(context, outputs_[i][0], &prep_output);
        const int output_depth = prep_filter->dims->data[0];
        const int accum_depth = prep_filter->dims->data[1];
        RuntimeShape prep_output_shape = GetTensorShape(prep_output);
        const int output_rank = prep_output_shape.DimensionsCount();
        const int batches = FlatSizeSkipDim(prep_output_shape, output_rank - 1);
        prepacked_dma_plans[i] = BuildPrepackedNodeDmaPlan(
            /*layer_t=*/0, roundUp(batches, 16), roundUp(output_depth, 16),
            roundUp(accum_depth, 64), -prep_input->params.zero_point,
            -prep_filter->params.zero_point, is_bias, preloaded_weights[i],
            preloaded_weight_sums[i], biases[i], crf_arr[i], crx_arr[i]);
  ...
  TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      if (builtin_code_[i] == kTfLiteBuiltinConv2d) { // CONV2D
        ...
        const bool isBias = (inputs_[i].size() == 3 && inputs_[i][2] >= 0);
        ...
        const int mode =
            vit_sim::SelectDriverMode(drv.layer_t, drv.pN, drv.pM, drv.pK,
                                      drv.is_bias);
        if (CanUsePrepackedNodeDma(prepacked_dma_plans[i], drv, mode)) {
          drv.prepacked_dma = &prepacked_dma_plans[i];
          drv.padded_weights = nullptr;
          drv.wt_sum = nullptr;
        } else {
          ensure_weight_cache(i, drv.M, drv.K, filter->data.int8, drv.pM,
                              drv.pK);
  ...
  std::vector<prepacked_node_dma> prepacked_dma_plans;
  //! H:
...
void TfLiteVitDelegateDelete(TfLiteDelegate *delegate) {
  ...
  SYSC_ON(profile.saveCSVRecords(".data/vit_sim_v8")); // in-fpga
#ifndef SYSC
  if (!dparams.unmap) {
