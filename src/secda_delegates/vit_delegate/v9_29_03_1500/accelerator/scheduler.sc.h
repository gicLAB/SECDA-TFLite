...
#pragma HLS resource core = AXI4LiteS metadata = "-bus_bundle slv0" variable = \
  ...
  while (1) {
    ...
    // Keep 4th output stream interface in HLS export when running with 3 cores.
    // This branch is not used in normal runs (_mode is expected 0/1).
    if (_mode < 0) {
      ADATA keepalive;
      keepalive.data = 0;
      keepalive.tlast = true;
      dout4.write(keepalive);
    }
