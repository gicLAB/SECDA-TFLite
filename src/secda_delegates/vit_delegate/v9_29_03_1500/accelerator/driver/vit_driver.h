...
void block_add(acc_container &drv) {
  ...
  if (mode == 0) { 
    for (int n = 0; n < drv.pN; n += HW_ROWS) {
      ...
      memcpy(input_buf, drv.padded_input + n * drv.pK, valid_bytes);
      if (n_rem < HW_ROWS) memset((int8_t*)input_buf + valid_bytes, 0, total_bytes - valid_bytes);
      
      // Start DMA 1 (Inputs) for the whole row
      drv.mdma->dmas[1].dma_start_send(total_bytes / 4);
      prf_end(2, drv.a_t->inp_copy);

      for (int m = 0; m < drv.pM; m += HW_DENSE_W) {
        prf_start(5);
        weight_len = 0; param_len = 0; bias_len = 0;
        ...
        prf_start(9);
        int recv_size_per_tile = HW_ROWS * HW_COLS / 4; 
        for (int c = 0; c < HW_CORES; c++) {
            drv.mdma->dmas[c].dma_start_recv(recv_size_per_tile);
        }

        // CRITICAL FIX: Only wait for the tile-specific DMAs to avoid 
        // deadlocking against DMA 1 (which is feeding the whole row)
        drv.mdma->dmas[2].dma_wait_send();
        drv.mdma->dmas[0].dma_wait_send();
        drv.mdma->dmas[3].dma_wait_send();

        for (int c = 0; c < HW_CORES; c++) {
            drv.mdma->dmas[c].dma_wait_recv();
            int8_t *output_val = (int8_t*)drv.mdma->dmas[c].dma_get_outbuffer();
            ...
            if (m_rem > 0) {
                for (int i = 0; i < n_rem; i++) {
                    int row_base = n * drv.pM + cur_m;
                    int row_offset_in_output = i * drv.pM;
                    memcpy(drv.padded_output + (row_base + row_offset_in_output),
                           output_val + (i * HW_COLS), 
                           m_rem);
                }
            }
        }
      ...
      // The row is finished, now we can safely wait for the input DMA to clear
  ...
  // =========================================================================
    ...
    for (int m = 0; m < drv.pM; m += HW_COLS) {
        ...
        for (int n = 0; n < drv.pN; n += HW_MOBILE_IN) {
            ...
            prf_start(9);
            int recv_size_per_core = HW_ROWS * HW_COLS / 4; 
            for (int c = 0; c < HW_CORES; c++) {
                drv.mdma->dmas[c].dma_start_recv(recv_size_per_core);
            }

            // CRITICAL FIX: Only wait for the active DMAs
            drv.mdma->dmas[1].dma_wait_send();
            drv.mdma->dmas[0].dma_wait_send();
            drv.mdma->dmas[3].dma_wait_send();
            ...
            for (int c = 0; c < HW_CORES; c++) {
                ...
                if (n_sub > 0) {
                    for (int i = 0; i < n_sub; i++) {
                        ...
                        memcpy(drv.padded_output + base, output_val + (i * HW_COLS), m_rem);
                    }
                }
            }
            prf_end(9, drv.a_t->out_copy);
