...
int ACCNAME::Quantised_Multiplier_conv(int x, int qm, int shift) {
  sc_int<64> local_pl;
  sc_int<32> local_pr;
  sc_int<32> local_msk;
  sc_int<32> local_sm;
...
#pragma HLS resource core = AXI4LiteS metadata = "-bus_bundle slv0" variable = \
  ...
  while (1) {
    ...
    for (int n = 0; n < pN; n += no_rows) {
      ...
      wait();
      // while(inpReadReadyS.read() == 1) wait(); //! a
      DWAIT(1);
      wait();
      ...
      for (int m = 0; m < pM; m += no_cols) {
        ...
        wait();
        // while(wgtReadReadyS.read() == 1) wait(); //! a

        // while (ppuReadyS.read() == 1) wait();

        biasReadReadyS.write(1);
        wait(); // a
        // while(biasReadReadyS.read() == 1) wait(); //! a
        while (inpReadReadyS.read() == 1 || wgtReadReadyS.read() == 1)
          wait(); //! r

        peReadyS.write(1);
        wait(); // ! Do not remove this, it breaks everything
        // while (peReadyS.read() == 1) wait(); //! a
    ...
    // no_inpR = 0;
    // cout << "no_wgts read / no_bias read / no_pe: " << no_wgtR << endl;
    // no_wgtR = 0;
  }
}