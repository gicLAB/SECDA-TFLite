...
#pragma HLS pipeline II = 1
        if (j < pmr) {
          ...
          value1 = Quantised_Multiplier(res[i][j + 0], crf_v[j+0], crx_v[j+0]);
          value2 = Quantised_Multiplier(res[i][j + 1], crf_v[j+1], crx_v[j+1]);
          value3 = Quantised_Multiplier(res[i][j + 2], crf_v[j+2], crx_v[j+2]);
          value4 = Quantised_Multiplier(res[i][j + 3], crf_v[j+3], crx_v[j+3]);
