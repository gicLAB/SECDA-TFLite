...
void block_add(acc_container &drv) {
  ...
  int param_len = 0;
  int *param_buf =
      drv.mdma->dmas[0].dma_get_inbuffer();
  ...
  //! H: Hardcoded for now - need to be changed to vectors
  //! Works for FC but only for FC
  // param_buf[param_len++] = drv.crx_a[0];
  // param_buf[param_len++] = drv.crf_a[0];
  ...
  for (int n = 0; n < drv.pN; n += NO_ROWS) {
    ...
    for (int m = 0; m < drv.pM; m += NO_COLS) {
      ...
      //+ =========================================================================
      //+ FIX: Copy the per-channel crx and crf values for the current block
      //+ =========================================================================
      //+ Copy the chunk of crf (multiplier) values for the current m_remaining channels
      memcpy(bias_buf + bias_len, drv.crf_a.data() + m, m_remaining * sizeof(int));
      bias_len += m_remaining;


      //- =========================================================================
      //- INCORRECT PACKING LOGIC
      //- memcpy(bias_buf + bias_len, drv.crx_a.data() + m, m_remaining * sizeof(int8_t));
      //- bias_len += (m_remaining * sizeof(int8_t) + 3) / 4; 
      //- =========================================================================

      //+ =========================================================================
      //+ FIX: Send each int8_t crx value as a full int to match hardware reads
      //+ =========================================================================
      for (int k = 0; k < m_remaining; ++k) {
        bias_buf[bias_len + k] = drv.crx_a[m + k];
      }
      bias_len += m_remaining;
      //+ =======================
