...
public:
  ...
  TfLiteStatus Init(TfLiteContext *context,
    ...
    //! H:
    crf_a.resize(params->nodes_to_replace->size);
    crx_a.resize(params->nodes_to_replace->size);
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      if (builtin_code_[i] == kTfLiteBuiltinConv2d) {
        Prepare_CONV2D_INT8(context, node, i, layers_params[i], opdatas[i],
                            ...
                            wt_sum3[i], wt_sum4[i], biases[i], crf_a[i], crx_a[i]);
  ...
  TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      if (builtin_code_[i] == kTfLiteBuiltinConv2d) { // CONV2D
        ...
        // store_unpad(padded_output, drv.N, drv.M, output->data.int8, 16,
                    // 16);
        // ai version:
  ...
  // std::vector<std::vector<int8_t>> crx;
    std::vector<std::vector<int>> crf_a;
  std::vector<std::vector<int8_t>> crx_a;
