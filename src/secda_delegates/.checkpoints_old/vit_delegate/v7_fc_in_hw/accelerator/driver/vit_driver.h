...
void block_add(acc_container &drv) {
  ...
  if(drv.layer_t == 0) {
    int crf = drv.crf; // Assuming all values are the same for padding calculation
    param_buf[param_len++] = crf;
    int crx = drv.crx; // Assuming all values are the same for padding calculation
  ...
  for (int n = 0; n < drv.pN; n += NO_ROWS) {
    ...
    for (int m = 0; m < drv.pM; m += NO_COLS) {
      ...
      //+ =========================================================================
      //+ FIX: Pack 4 int8_t crx values into a single int
      //+ =========================================================================
      memcpy(bias_buf + bias_len, drv.crx_a.data() + m, m_remaining * sizeof(int8_t));
      // bias_len += (m_remaining + 3) / 4;
