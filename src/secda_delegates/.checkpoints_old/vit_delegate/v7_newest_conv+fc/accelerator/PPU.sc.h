...
#pragma HLS UNROLL
          ...
          int local_crx_fc = crx;
          int local_crf_fc = crf;
          int local_res_fc[4];
          int local_pl_fc;
          int local_pr_fc;
          int local_msk_fc;
...
#pragma HLS array_partition variable = local_res_fc complete dim = 0
          for (int c = 0; c < 4; c++) {
#pragma HLS UNROLL
            local_res_fc[c] = res[i][j + c];
          }
          // FC layer
          if (local_crx_fc > 0) {
            pl = local_crx_fc;
            ...
            pl = 1;
            pr = -local_crx_fc;
            msk = (1 << -local_crx_fc) - 1;
            sm = msk >> 1;
          }
          wait();
          value1 = Quantised_Multiplier_FC(local_res_fc[0], local_crf_fc, local_crx_fc);
          value2 = Quantised_Multiplier_FC(local_res_fc[1], local_crf_fc, local_crx_fc);
          value3 = Quantised_Multiplier_FC(local_res_fc[2], local_crf_fc, local_crx_fc);
          value4 = Quantised_Multiplier_FC(local_res_fc[3], local_crf_fc, local_crx_fc);
          wait();
        }
        wait(); // a
