...
#pragma HLS pipeline II = 1
        if (j < pmr) {
          ...
          int cur_crx_1 = crx_v[j+0];
          int cur_crx_2 = crx_v[j+1];
          int cur_crx_3 = crx_v[j+2];
          int cur_crx_4 = crx_v[j+3];
          wait();

          int cur_crf_1 = crf_v[j+0];
          int cur_crf_2 = crf_v[j+1];
          int cur_crf_3 = crf_v[j+2];
          int cur_crf_4 = crf_v[j+3];
          wait();



          value1 = Quantised_Multiplier(res[i][j + 0], cur_crf_1, cur_crx_1);
          value2 = Quantised_Multiplier(res[i][j + 1], cur_crf_2, cur_crx_2);
          value3 = Quantised_Multiplier(res[i][j + 2], cur_crf_3, cur_crx_3);
          value4 = Quantised_Multiplier(res[i][j + 3], cur_crf_4, cur_crx_4);
          wait();
