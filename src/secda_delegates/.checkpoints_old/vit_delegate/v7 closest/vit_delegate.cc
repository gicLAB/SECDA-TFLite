  ...
  TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      if (builtin_code_[i] == kTfLiteBuiltinConv2d) { // CONV2D
        ...
        // store_unpad(padded_output, out_batches, out_cols, output->data.int8, 16, 16); 
        ...
        store_unpad(padded_output, drv.N, drv.M, output->data.int8, 16, 16); // the last 2 are the rfactor above
        delete[] padded_input;
