...
int ACCNAME::Quantised_Multiplier(int x, sc_int<32> qm, sc_int<8> shift) {
  int nshift = shift;
...
//     local_pr = -shift;
//     local_msk = (1 << -shift) - 1;
//     local_sm = local_msk >> 1;
//   }
//   sc_int<64> val = x * local_pl;
...
//   val_3 = val_3 >> local_pr;
//   sc_int<32> temp_2 = temp_1 & local_msk;
//   sc_int<32> temp_3 = (temp_1 < 0) & 1;
//   sc_int<32> temp_4 = local_sm + temp_3;
...
if(shift > 0) {
  ...
  wait();
  wait();
  sc_int<64> val = mul_s64(x, local_pl);
  wait(); // Pipeline register after first multiplication
  wait(); //!a
  wait(); //!a2
  wait();
  wait();
  ...
  wait(); // Pipeline register after clamping
  wait(); //!a
  wait(); //!a2
  wait();
  wait();
  ...
  wait(); // Pipeline register after second multiplication
  wait(); // Additional pipeline stage for complex arithmetic
  wait(); //!a
  wait(); //!a2
  wait();
  wait();
  ...
  temp_1 = (val_2 + POS) / DIVMAX;
  if (val_2 < 0) temp_1 = (val_2 + NEG) / DIVMAX;
  wait(); // Pipeline register after division
  wait(); //!a
  wait(); //!a2
  wait();
  wait();
  ...
  sc_int<32> temp_4 = local_sm + temp_3;
  sc_int<32> temp_5 = ((temp_2 > temp_4) & 1);
  wait(); // Pipeline register before final operation
  wait(); //!a
  wait(); //!a2
  wait();
  wait();
