...
SC_MODULE(ACCNAME) {
  ...
  // sc_int<8> rows[pnF][1536]; // vit: 768,  deit:
  // sc_int<8> cols[pmF][1536];
...
#else
  int Quantised_Multiplier(int, int, sc_int<8>); // Defined in "compute.sc.h"
...
#pragma HLS array_partition variable = crf_v cyclic factor = 4 dim = 1
#pragma HLS array_partition variable = crx_v cyclic factor = 4 dim = 1
