...
void log_padded_output_to_file(acc_container &drv, const std::string &filename_prefix = "vit_output") {
  static int call_count = 0;
  call_count++;

  std::string filename = "debugging/" + filename_prefix + "_layer" + std::to_string(drv.layer) +
                        "_call" + std::to_string(call_count) + ".txt";
  ...
  outfile << "Padded Dimensions: pN=" << drv.pN << ", pM=" << drv.pM << ", pK=" << drv.pK << "\n";
