void ACCNAME::PPU() {
  ...
  while (1) {
    ...
    for (int i = 0; i < pnr_ppu; i++) {
      DWAIT(1);
      for (int j = 0; j < pmr_ppu; j += 4) {
        ...
        crx4 = crx_v[j + 3];
          ...
          wait();
          value1 =
              Quantised_Multiplier(res[i][j + 0], crf1, crx1);
            wait();
          value2 =
              Quantised_Multiplier(res[i][j + 1], crf2, crx2);
              wait();
          value3 =
              Quantised_Multiplier(res[i][j + 2], crf3, crx3);
              wait();
          value4 =
              Quantised_Multiplier(res[i][j + 3], crf4, crx4);
          wait();
