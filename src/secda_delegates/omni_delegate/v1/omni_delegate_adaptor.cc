...
TfLiteDelegate *CreateOmniDelegateFromOptions(char **options_keys,
                                              char **options_values,
                                              size_t num_options) {
  OmniDelegateOptions options = TfLiteOmniDelegateOptionsDefault();

  // Parse key-values options to OmniDelegateOptions by mimicking them as
  // command-line flags.
  std::vector<const char *> argv;
  argv.reserve(num_options + 1);
  constexpr char kOmniDelegateParsing[] = "omni_delegate_parsing";
  argv.push_back(kOmniDelegateParsing);
  ...
  return TfLiteOmniDelegateCreate(&options);
}
...
tflite_plugin_create_delegate(char **options_keys, char **options_values,
  ...
  return tflite::tools::CreateOmniDelegateFromOptions(
      options_keys, options_values, num_options);
}

TFL_CAPI_EXPORT void tflite_plugin_destroy_delegate(TfLiteDelegate *delegate) {
  TfLiteOmniDelegateDelete(delegate);
}
