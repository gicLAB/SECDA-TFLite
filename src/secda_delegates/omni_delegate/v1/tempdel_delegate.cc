      ...
      if (builtin_code_[i] == kTfLiteBuiltinAdd) { // ADD
        ...
        TfLiteTensor *transposed_weights =
            data->weights_are_transposed
                ? GetTempdelorary(context, node, data->transposed_weights_index)
                : nullptr;

        TfLiteTensor *col2im =
            data->has_col2im ? GetTempdelorary(context, node, data->col2im_index)
                             : nullptr;

        TfLiteTensor *scratch_buffer;
        TF_LITE_ENSURE_OK(context, GetTempdelorarySafe(context, node,
        ...
        GetTempdelorarySafe(context, node, scratch_index, &tempdel_index);
        GetTempdelorarySafe(context, node, resolved_axis_index, &resolved_axis);
        GetTempdelorarySafe(context, node, tempdel_accum_index, &tempdel_sum);
        GetTempdelorarySafe(context, node, normalized_dims_index,
                         &normalized_dims);

        // Resize the output tensor if the output tensor is dynamic.
        if (IsDynamicTensor(op_context.output)) {
          TF_LITE_ENSURE_OK(
              context, ResizeTempdelAxis(context, &op_context, resolved_axis));
          TF_LITE_ENSURE_OK(context, ResizeOutputTensor(context, &op_context));
          TF_LITE_ENSURE_OK(context,
                            ResizeTempdelAccum(context, &op_context, tempdel_sum));
        }

        if (IsDynamicTensor(normalized_dims)) {
          TF_LITE_ENSURE_OK(
              context, ResizeTempdelDims(context, &op_context, normalized_dims));
        }
