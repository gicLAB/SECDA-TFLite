...
TfLiteDelegate *CreateTempdelDelegateFromOptions(char **options_keys,
                                              char **options_values,
                                              size_t num_options) {
  TempdelDelegateOptions options = TfLiteTempdelDelegateOptionsDefault();

  // Parse key-values options to TempdelDelegateOptions by mimicking them as
  // command-line flags.
  std::vector<const char *> argv;
  argv.reserve(num_options + 1);
  constexpr char kTempdelDelegateParsing[] = "tempdel_delegate_parsing";
  argv.push_back(kTempdelDelegateParsing);
  ...
  return TfLiteTempdelDelegateCreate(&options);
}
...
tflite_plugin_create_delegate(char **options_keys, char **options_values,
                              size_t num_options,
                              void (*report_error)(const char *)) {
  return tflite::tools::CreateTempdelDelegateFromOptions(
      options_keys, options_values, num_options);
}

TFL_CAPI_EXPORT void tflite_plugin_destroy_delegate(TfLiteDelegate *delegate) {
  TfLiteTempdelDelegateDelete(delegate);
}
