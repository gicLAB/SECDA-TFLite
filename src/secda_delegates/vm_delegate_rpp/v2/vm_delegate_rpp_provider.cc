...
public:
  VMDelegateProvider() {
    default_params_.AddParam("use_vm_delegate_rpp",
...
std::vector<Flag> VMDelegateProvider::CreateFlags(ToolParams *params) const {
  std::vector<Flag> flags = {
      CreateFlag<bool>("use_vm_delegate_rpp", params,
                       "use the vm delegate."),
...
void VMDelegateProvider::LogParams(const ToolParams &params,
                                      bool verbose) const {
  LOG_TOOL_PARAM(params, bool, "use_vm_delegate_rpp", "Use vm test delegate",
                 verbose);
...
VMDelegateProvider::CreateTfLiteDelegate(const ToolParams &params) const {
  if (params.Get<bool>("use_vm_delegate_rpp")) {
...
VMDelegateProvider::CreateRankedTfLiteDelegate(
  ...
  return std::make_pair(std::move(ptr),
                        params.GetPosition<bool>("use_vm_delegate_rpp"));
}
} // namespace tools
} // namespace tflite
