...
struct acc_container {
  ...
  bool Start_Transfer() {
    ...
    mdma->multi_dma_change_start(dfs[0].dbuf_set[s_buf].offset);
  ...
  void Set_Results() {
    ...
    // mdma->dmas[0].dma_start_recv(recv_len);
    dsr.cID++;
  }

  void Recieve_Results() { 
    // mdma->multi_dma_wait_recv_4(); 
    mdma->dmas[0].dma_wait_recv();
  }
};
