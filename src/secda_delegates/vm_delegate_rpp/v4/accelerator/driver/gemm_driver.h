...
void Config_Acc(acc_container &drv) {
  ...
  drv.mdma->multi_dma_change_start(0);
...
void Load_Input_Data(acc_container &drv, int start_row, int rows_step,
  ...
  int inp_size = rrow_steps * rdepth / 16;
  ...
  for (int c = 0; c < rows_step; c += 4) {
    for (int i = 0; i < rdepth / 4; i++) {
      ...
      in0[inl0++] = drv.inb_0[i + drv.in_id];
      in0[inl0++] = drv.inb_1[i + drv.in_id];
      in0[inl0++] = drv.inb_2[i + drv.in_id];
      in0[inl0++] = drv.inb_3[i + drv.in_id];
    }
    drv.in_id += rdepth / 4;
  }
#else
  ...
  for (int r = 0; r < rows_step; r += 4) {
    ...
    for (int i = 0; i < rdepth / 4; i += 4) {
      ...
      vst1q_s32(in0 + inl0, tmp0);
      inl0 += 4;
      vst1q_s32(in0 + inl0, tmp1);
      inl0 += 4;
      vst1q_s32(in0 + inl0, tmp2);
      inl0 += 4;
      vst1q_s32(in0 + inl0, tmp3);
      inl0 += 4;
    }
    drv.in_id += rdepth / 4;
  }
...
void Load_Weight_Data(acc_container &drv, int free_buf, int8_t *results,
  ...
  int wgt_size = rcols_step * rdepth_step / 16;
  ...
  for (int i = 0; i < wsend_full; i++) {
    ...
    in0[inl0++] = drv.wb_0[w_dex + i];
    in0[inl0++] = drv.wb_1[w_dex + i];
    in0[inl0++] = drv.wb_2[w_dex + i];
    in0[inl0++] = drv.wb_3[w_dex + i];
  }
  // cout << "here3" << endl;
#else
  for (int i = 0; i < wsend_neon_len; i += 4) {
    ...
    vst1q_s32(in0 + inl0, vld1q_s32(drv.wb_0 + w_dex + i));
    inl0 += 4;
    vst1q_s32(in0 + inl0, vld1q_s32(drv.wb_1 + w_dex + i));
    inl0 += 4;
    vst1q_s32(in0 + inl0, vld1q_s32(drv.wb_2 + w_dex + i));
    inl0 += 4;
    vst1q_s32(in0 + inl0, vld1q_s32(drv.wb_3 + w_dex + i));
    inl0 += 4;
  }
  for (int i = wsend_neon_len; i < wsend_full; i++) { // Bit shift specific code
    // in0[inl0++] = drv.wb_0[w_dex + i];
    // in1[inl1++] = drv.wb_1[w_dex + i];
    // in2[inl2++] = drv.wb_2[w_dex + i];
    // in3[inl3++] = drv.wb_3[w_dex + i];
...
void Store_Results2(acc_container &drv) {
  prf_start(1);
  ...
  cout << "r_buf: " << r_buf << " offset: " << offset << endl;
...
void Store_Results(acc_container &drv) {
  ...
  for (int i = 0; i < drows; i += 4) {
    for (int j = 0; j < cols_step; j++) {
      // cout << bo0[out0] << " " << bo0[out1] << " " << bo0[out2] << " "
      //      << bo0[out3] << endl;

      file << (int)bo0[out0] << " " << (int)bo0[out1] << " " << (int)bo0[out2]
           << " " << (int)bo0[out3] << endl;
      base[(i + 0) * output_stride + j] = bo0[out0++];
      base[(i + 1) * output_stride + j] = bo0[out1++];
      base[(i + 2) * output_stride + j] = bo0[out2++];
      base[(i + 3) * output_stride + j] = bo0[out3++];

      if (out0 % 4 == 0) out0 += (4 * 3);
      if (out1 % 4 == 0) out1 += (4 * 3);
      if (out2 % 4 == 0) out2 += (4 * 3);
      if (out3 % 4 == 0) out3 += (4 * 3);
    }
    out0 += colsr;
    out1 += colsr;
    out2 += colsr;
  ...
  for (int i = 0; i < drows; i += 4) {
    ...
    for (int j = 0; j < unrolled_cols; j += 16) {
      ...
      out0 += 16;
      tmp1 = vld1q_s8(bo0 + out0);
      out0 += 16;
      tmp2 = vld1q_s8(bo0 + out0);
      out0 += 16;
      tmp3 = vld1q_s8(bo0 + out0);
      out0 += 16;
  ...
  if ((rows_step % 4) == 3) {
    for (int j = 0; j < cols_step; j++) {
      ...
      base[(drows + 1) * output_stride + j] = bo0[out1++];
      base[(drows + 2) * output_stride + j] = bo0[out2++];
...
void Load_Weight_Compute_Store(acc_container &drv, int8_t *results,
  ...
  // cout << "Load_Weight_Data Start" << endl;
  Load_Weight_Data(drv, free_buf, results, output_stride, c, rcols_step, r,
                   rrows_step, rdepth_step, rows_step, cols_step);
  // cout << "Load_Weight_Data Done" << endl;
  prf_start(1);
  drv.Start_Transfer();
  prf_end(1, drv.t2.send_weights);
  // cout << "Start_Transfer" << endl;

  prf_start(2);
  drv.Set_Results();
  prf_end(2, drv.t2.set_results);
  // cout << "Set_Results" << endl;

  prf_start(3);
  Start_Compute(drv, rrows_step, rcols_step);
  prf_end(3, drv.t2.start_compute);
  // cout << "Start_Compute" << endl;

  prf_start(4);
  drv.Recieve_Results();
  prf_end(4, drv.t2.receive_results);

  // cout << "Recieve_Results" << endl;
  SYSC_ON(drv.profile->saveProfile(drv.acc->profiling_vars));
  Store_Results(drv);
  // cout << "Store Results Done" << endl;
}
...
void TileGEMM(acc_container &drv, int output_stride, int depth, int rdepth,
  ...
  int col_inc = std::min(std::min(rcols, max_cols), (WSUMS_BUF_LEN * 4));
...
void Entry(acc_container &drv, int8_t *dst) {
  ...
  // print out no of tiles
  cerr << "Layer: " << drv.t.layer
       << " Weight Tiles: " << drv.t.layer_weight_tile
       << " Input Tiles: " << drv.t.layer_input_tile << endl;
