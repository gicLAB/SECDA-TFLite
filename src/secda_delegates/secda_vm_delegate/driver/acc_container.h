...
struct acc_container {
  ...
  void memRW(D *dst, int d_dex, S *src, int s_dex, int ins) {
    ...
    ofstream file;
    std::string filename = "./traces/" + std::to_string(t.layer) + ".trace";
    file.open(filename, std::ios_base::app);
    file << ins << " " << (uint32_t)(src_addr) << " " << (uint32_t)(dst_addr)
         << endl;
    file.close();
  }

  template <typename D>
  void memR(D *dst, int d_dex, int ins) {
    auto dst_addr = (void *)(&dst[d_dex]);
    ofstream file;
    std::string filename = "./traces/" + std::to_string(t.layer) + ".trace";
    file.open(filename, std::ios_base::app);
    file << ins << " " << (uint32_t)(dst_addr) << endl;
    file.close();
  }
