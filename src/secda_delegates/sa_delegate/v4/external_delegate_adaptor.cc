...
TfLiteDelegate* CreateSADelegateFromOptions(char** options_keys,
                                               char** options_values,
                                               size_t num_options) {
  SADelegateOptions options = TfLiteSADelegateOptionsDefault();

  // Parse key-values options to SADelegateOptions by mimicking them as
  // command-line flags.
  std::vector<const char*> argv;
  argv.reserve(num_options + 1);
  constexpr char kSADelegateParsing[] = "sa_delegate_parsing";
  argv.push_back(kSADelegateParsing);
  ...
  TFLITE_LOG(INFO) << "SA delegate: allowed_builtin_code set to "
                   << options.allowed_builtin_code << ".";
  TFLITE_LOG(INFO) << "SA delegate: error_during_init set to "
                   << options.error_during_init << ".";
  TFLITE_LOG(INFO) << "SA delegate: error_during_prepare set to "
                   << options.error_during_prepare << ".";
  TFLITE_LOG(INFO) << "SA delegate: error_during_invoke set to "
                   << options.error_during_invoke << ".";

  return TfLiteSADelegateCreate(&options);
}
...
TFL_CAPI_EXPORT TfLiteDelegate* tflite_plugin_create_delegate(
  ...
  return tflite::tools::CreateSADelegateFromOptions(
      options_keys, options_values, num_options);
}

TFL_CAPI_EXPORT void tflite_plugin_destroy_delegate(TfLiteDelegate* delegate) {
  TfLiteSADelegateDelete(delegate);
}
