...
public:
  VMDelegateProvider() {
    default_params_.AddParam("use_vm_delegate",
...
std::vector<Flag> VMDelegateProvider::CreateFlags(ToolParams *params) const {
  std::vector<Flag> flags = {
      CreateFlag<bool>("use_vm_delegate", params,
                       "use the vm delegate."),
...
void VMDelegateProvider::LogParams(const ToolParams &params,
                                      bool verbose) const {
  LOG_TOOL_PARAM(params, bool, "use_vm_delegate", "Use vm test delegate",
                 verbose);
...
VMDelegateProvider::CreateTfLiteDelegate(const ToolParams &params) const {
  if (params.Get<bool>("use_vm_delegate")) {
...
VMDelegateProvider::CreateRankedTfLiteDelegate(
  ...
  return std::make_pair(std::move(ptr),
                        params.GetPosition<bool>("use_vm_delegate"));
}
} // namespace tools
} // namespace tflite
