...
void LoadWeight(acc_container &drv, int starting_row, int number_of_rows,
                int padded_depth, int filter_step, int starting_filter, int row_per_filter) {
  int *in0 = drv.mdma->dmas[0].dma_get_inbuffer();
  ...
  for (int i = 0; i < number_of_rows; i++) {
    ...
    for (int j = 0; j < padded_depth_4; j+=4) {
      in0[inl0++] = drv.loaded_weights[src_addr + j + 0];
      in1[inl1++] = drv.loaded_weights[src_addr + j + 1];
      in2[inl2++] = drv.loaded_weights[src_addr + j + 2];
      in3[inl3++] = drv.loaded_weights[src_addr + j + 3];
    }
...
void TileMM2IM(acc_container &drv, int padded_depth) {
  ...
  for (; o_3 < drv.oc; o_3 += filter_step) {
    ...
    LoadWeight(drv, o_3 * cols_per_filter, fs_rem * cols_per_filter,
               padded_depth, fs_rem, o_3, cols_per_filter);
