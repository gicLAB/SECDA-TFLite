...
public:
  ...
  TfLiteStatus Init(TfLiteContext *context,
    ...
    if (!dparams.init) {
      ...
      DTOG(std::cerr << "MM2IM Accelerator v12");
#ifdef ACC_NEON
    ...
    crx_scale.resize(params->nodes_to_replace->size);
  ...
  TfLiteStatus Prepare(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      crx_scale[i].resize(channels_out);
      ...
      for (int j = 0; j < channels_out; j++) {
        ...
        const float scale =
            is_per_channel ? filter_scales[j] : filter_scales[0];
        const double filter_scale = static_cast<double>(scale);
        const double effective_output_scale = static_cast<double>(input_scale) *
                                              filter_scale /
                                              static_cast<double>(output_scale);
        crx_scale[i][j] = effective_output_scale;
      }
      ...
      preloader.dma_preload(loaded_wgts, kernel_size, filters, rounded_depth,
                            rhs_offset, acc_wt_sum, &crf[i][0],
                            &crx_scale[i][0], &crx_8[i][0], bias_send);
      if (preloader.layer == dparams.delegated_nodes ||
        ...
        // DTOG(std::cerr << "Allocated size: " << preloader.alloced_size
        //                << std::endl);
  ...
  TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      bool input_preloaded = preload_inputs_dma(
          input_data, par->depth, par->cols, mdma.dmas[0].dma_get_inbuffer());
...
// CPU implemented here does provide correct results
...
#endif
      dparams.layer++;
      if (dparams.layer % dparams.delegated_nodes == 0) p_t.call_count++;
  ...
  std::vector<std::vector<double>> crx_scale;
