...
#define dma_in0 0x18100000
// #define dma_out0 0x18800000
#define dma_out0 0x1D000000
#define DMA_BL 4194304

#define DMA_IN_BUF_SIZE 0x4000000
// #define DMA_IN_BUF_SIZE 4194304

#define DMA_OUT_BUF_SIZE 0x0400000

#define DMA_INS_SIZE 0x0000100

// #define DMA_INP_SIZE 0x0400000
#define DMA_INP_SIZE 0x1000000
#define DMA_INP_OFFSET DMA_INS_SIZE

#define DMA_WGT_SIZE (DMA_IN_BUF_SIZE - DMA_INP_SIZE - DMA_INS_SIZE)
#define DMA_WGT_OFFSET (DMA_INP_SIZE + DMA_INS_SIZE)

#define DMA_RANGE_START 0x18100000
#define DMA_RANGE_END 0x1fffffff
#define DMA_RANGE_SIZE 0x7F00000
#endif
...
// #define PE_ACC_BUF_SIZE 1024 * 16
...
struct PE_vars {
  ...
  sc_signal<double, SC_MANY_WRITERS> crx_scale_data;
  ...
  sc_signal<double> crx_scale_data;
  ...
  PE_vars(int size, int sid)
      : online((std::string("online") + std::to_string(sid)).c_str()),
        ...
        crx_scale_data((std::string("crx_scale_data") + std::to_string(sid)).c_str()),
  ...
  PE_vars(int size)
      : online("online"), compute("compute"), reset_compute("reset_compute"),
        ...
        crx_scale_data("crx_scale_data"), ra_data("ra_data"), send("send"),
        cols_per_filter("cols_per_filter"), depth("depth"),
        compute_done("compute_done"), wgt_loaded("wgt_loaded"),
        send_done("send_done"), process_cal("process_cal"),
        process_cal_done("process_cal_done"), oh("oh"), ow("ow"),
        kernel_size("kernel_size"), stride_x("stride_x"), stride_y("stride_y"),
        pt("pt"), pl("pl"), width_col("width_col"), crow("crow"),
        num_rows("num_rows"), wgt_sum_fifo(size), wgt_fifo(size),
