...
int PE::Quantised_Multiplier_ruy_reference(int x, int qm, sc_int<8> shift) {
  ...
  // sc_int<64> result = mul_s64(x_64, quantized_multiplier_64) + round;
...
// #else
        ...
        // int qm_ret1 = ra + Quantised_Multiplier_gemmlowp(acc_store[dex] + bias,
        //                                                 crf, crx.range(7, 0));
// #endif
        // double qm_ret_double =  std::round((acc_store[dex] + bias) * crx_scale);
        double qm_ret_double = (acc_store[dex] + bias) * crx_scale;
        sc_int<32> qm_ret = qm_ret_double + ra;
        // sc_int<32> qm_ret = ra + acc_store[dex] + bias;
