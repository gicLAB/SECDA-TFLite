...
SC_MODULE(ACCNAME) {
  ...
  sc_int<8> rows[NUM_CORES][pn_block][1024];

  // Weight Buffer: 4 Banks of [64 cols][1024 depth]
  sc_int<8> cols[NUM_CORES][pm_block][1024];
...
// --- KEY FIX: Reduce Factor to 16 ---
// 4 Cores * 16 Partitions = 64 BRAMs.
// Even with overhead, this will be well under your 280 limit.

#pragma HLS array_partition variable = rows complete dim = 1
#pragma HLS array_partition variable = rows cyclic factor = 16 dim = 3

#pragma HLS array_partition variable = cols complete dim = 1
#pragma HLS array_partition variable = cols cyclic factor = 16 dim = 3

// --- Result Buffers ---
// Keep these at factor 4, as they don't consume much BRAM
#pragma HLS array_partition variable = res complete dim = 1
#pragma HLS array_partition variable = res cyclic factor = 4 dim = 3

#pragma HLS array_partition variable = prec complete dim = 1
#pragma HLS array_partition variable = prec cyclic factor = 4 dim = 3

// --- Small Control Arrays (Keep Complete) ---
