...
void block_add(acc_container &drv) {
  ...
  const int HW_ROWS = 64;  
  const int HW_COLS = 64;  
  const int HW_CORES = 4;
        ...
        if (drv.layer_t == 1) {
            for (int c = 0; c < HW_CORES; c++) {
                 ...
                 if (m_rem > 0) {
                      ...
                      if (m_rem < HW_COLS) memset((int8_t*)(bias_buf + bias_len) + m_rem, 0, HW_COLS - m_rem);
                 } else {
                      memset(bias_buf + bias_len, 0, HW_COLS); 
                 }
                 bias_len += HW_COLS / 4;
            }
        }
