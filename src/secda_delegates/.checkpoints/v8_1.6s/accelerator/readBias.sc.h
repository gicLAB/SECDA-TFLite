  ...
  while (1) {
    ...
    if (layer_t == 1) {
      if (current_mode == MODE_DENSE) {
         ...
         for (int a = 0; a < pm_block; a += 4) {
            int v0 = din1->read().data; int v1 = din1->read().data;
            int v2 = din1->read().data; int v3 = din1->read().data;
            ADATA d_crx = din4->read();
            for (int core = 0; core < NUM_CORES; core++) {
               crf_v[core][a+0] = v0; crf_v[core][a+1] = v1;
               crf_v[core][a+2] = v2; crf_v[core][a+3] = v3;
               crx_v[core][a+0] = d_crx.data.range(7,0);
               crx_v[core][a+1] = d_crx.data.range(15,8);
               crx_v[core][a+2] = d_crx.data.range(23,16);
               crx_v[core][a+3] = d_crx.data.range(31,24);
       ...
       for (int a = 0; a < pn_block; a++) {
          ...
          for (int core = 0; core < NUM_CORES; core++) {
             ...
             for (int b = 0; b < pm_block; b++) 
                prec[core][a][b] = bias[core][b] + wt_sum[core][b] + i_sum;
          }
       }
    } else {
       for (int core = 0; core < NUM_CORES; core++) {
