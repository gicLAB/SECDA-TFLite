...
SC_MODULE(ACCNAME) {
  ...
  sc_int<8> rows[pnF][1024]; // vit: 768,  deit:
  sc_int<8> cols[pmF][1024];
  int temp[pkF][4];
...
// Partitioning 'bias' and 'wt_sum' is required for the unrolled compute loop
  #pragma HLS ARRAY_PARTITION variable=bias complete dim=1
  #pragma HLS ARRAY_PARTITION variable=wt_sum complete dim=1

  // Partitioning 'prec' on dim 2 is required for the *inner* unrolled loop
  #pragma HLS ARRAY_PARTITION variable=prec complete dim=2

  // Partitioning 'crf_v' and 'crx_v' supports 4 parallel writes in the II=1 loop
