#include "tensorflow/lite/delegates/utils/secda_delegates/vit_delegate/v7/vit_delegate.h"
#include <fstream>
#include <iostream>
#include <utility>
...
#endif
      std::cout << "VIT_ACC Accelerator v7";
#ifdef ACC_NEON
      std::cout << " with Neon";
#endif
      std::cout << std::endl;
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      if (builtin_code_[i] == kTfLiteBuiltinConv2d) {
        Prepare_CONV2D_INT8(context, node, i, layers_params[i], opdatas[i],
                            ...
                            wt_sum3[i], wt_sum4[i], biases[i], crf_arr[i], crx_arr[i]);
  ...
  TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      if (builtin_code_[i] == kTfLiteBuiltinConv2d) { // CONV2D
        ...
        drv.M = filter_rows;
        drv.K = filter_cols;
        cout << "unpadded N: " << drv.N << ", M: " << drv.M
             << ", K: " << drv.K << endl;
        drv.layer_t = 1; // CONV layer

        // ===== LAYER GEOMETRY =====
        cout << "\n========== LAYER GEOMETRY ==========" << endl;
        cout << "Input Dimensions (H, W, Cin): " << input_height << ", " 
             << input_width << ", " << input_depth << endl;
        cout << "Output Dimensions (H, W, Cout): " << output_height << ", " 
             << output_width << ", " << output_channel << endl;
        cout << "Filter/Kernel (Kh, Kw): " << filter_height << ", " 
             << filter_width << endl;
        cout << "Stride (H, W): " << stride_height << ", " << stride_width << endl;
        cout << "Padding Type: " << (params->padding == kTfLitePaddingValid ? "VALID" : "SAME") << endl;
        cout << "Padding Height: " << op_params.padding_values.height << endl;
        cout << "Padding Width: " << op_params.padding_values.width << endl;

        // ===== QUANTIZATION PARAMETERS =====
        cout << "\n======= QUANTIZATION PARAMETERS =======" << endl;
        cout << "Input Zero Point (rhs_offset): " << -input->params.zero_point << endl;
        cout << "Weight Zero Point (lhs_offset): " << -filter->params.zero_point << endl;
        cout << "Output Zero Point (ra): " << output->params.zero_point << endl;
        cout << "Input Scale: " << input->params.scale << endl;
        cout << "Weight Scale: " << filter->params.scale << endl;
        cout << "Output Scale: " << output->params.scale << endl;
        ...
        cout << "\n===== HARDWARE BLOCK SIZES =====" << endl;
        cout << "N vs pN (output cols): " << drv.N << " vs " << drv.pN << endl;
        cout << "M vs pM (output channels): " << drv.M << " vs " << drv.pM << endl;
        cout << "K vs pK (input channels * Kh * Kw): " << drv.K << " vs " << drv.pK << endl;
        
        cout << "padded N: " << drv.pN << ", M: " << drv.pM
             << ", K: " << drv.pK << endl;
        cout << "====================================\n" << endl;
        ...
        // ai version:
        // DOes not work in sw
        // store_unpad(padded_output, out_batches, out_cols, output->data.int8, 16,
                    // 16);
        // other AI version
        // Works in SW
        ...
        // ===== LAYER GEOMETRY =====
        cout << "\n========== LAYER GEOMETRY ==========" << endl;
        cout << "Input Dimensions (Batch, Features): " << batches << ", " 
             << accum_depth << endl;
        cout << "Output Dimensions (Batch, Features): " << batches << ", " 
             << output_depth << endl;
        cout << "Filter/Kernel Dimensions: 1x1" << endl;
        cout << "Stride: 1, 1" << endl;
        cout << "Padding: None" << endl;
        ...
        // ===== HARDWARE BLOCK SIZES =====
        cout << "\n===== HARDWARE BLOCK SIZES =====" << endl;
        cout << "N vs pN (batch size): " << drv.N << " vs " << drv.pN << endl;
        cout << "M vs pM (output depth): " << drv.M << " vs " << drv.pM << endl;
        cout << "K vs pK (accumulation depth): " << drv.K << " vs " << drv.pK << endl;
        cout << "====================================\n" << endl;
        ...
        drv.crx_a = crx_arr[i]; 
        drv.crf_a = crf_arr[i];
        // make drv.crx_a and drv.crf_a vectors of output_shift and output_multiplier
          // drv.crx_a = std::vector<int8_t>(drv.pM, output_shift);
          // drv.crf_a = std::vector<int>(drv.pM, output_multiplier);
        drv.ra = output_offset;
        drv.rhs_offset = input_offset;
        drv.lhs_offset = filter_offset;
        drv.a_t = &a_t;
    ...
    if (delegated_node && isCONV2D) {
      ...
      if (groups > 1) {
        ...
        std::cout << "CONV2D with groups=" << groups << " (output tensor " 
                  << node->outputs->data[0] << ") not delegated (grouped convolution not supported)" << std::endl;
      }
    }
