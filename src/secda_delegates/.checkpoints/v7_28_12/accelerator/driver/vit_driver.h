...
void block_add(acc_container &drv) {
  ...
  // PRINT lhs offset and rhs offset
  std::cout << "[VIT v7 Driver] Layer " << drv.layer << " (layer_t=" << drv.layer_t
            << "): lhs_offset=" << drv.lhs_offset
            << ", rhs_offset=" << drv.rhs_offset << std::endl;
  ...
  int NO_ROWS = pn_block;
  int NO_COLS = pm_block;
  ...
  if (drv.layer_t == 0) {
    ...
    // std::cout << "[VIT v7 Driver] Layer " << drv.layer << " (layer_t=0): CRF=" << crf << ", CRX=" << static_cast<int>(crx) << std::endl;
  }
  ...
  for (int n = 0; n < drv.pN; n += NO_ROWS) {
    ...
    for (int m = 0; m < drv.pM; m += NO_COLS) {
      ...
      if (drv.layer_t == 1) {
        ...
        // Copy CRF per-channel multipliers (int32 per channel)
        memcpy(param_buf + param_len, drv.crf_a.data() + m,
               m_remaining * sizeof(int));
        param_len += m_remaining;

        // Pack CRX int8 shifts into int32 words (4 per word), pad tail with zeros
        {
          int packed_count = (m_remaining + 3) / 4;
          for (int i = 0; i < packed_count; i++) {
            int idx = m + i * 4;
            int8_t w0 = (idx + 0 < m + m_remaining) ? drv.crx_a[idx + 0] : 0;
            int8_t w1 = (idx + 1 < m + m_remaining) ? drv.crx_a[idx + 1] : 0;
            int8_t w2 = (idx + 2 < m + m_remaining) ? drv.crx_a[idx + 2] : 0;
            int8_t w3 = (idx + 3 < m + m_remaining) ? drv.crx_a[idx + 3] : 0;
            int packed = (static_cast<unsigned char>(w0) << 0) |
                         (static_cast<unsigned char>(w1) << 8) |
                         (static_cast<unsigned char>(w2) << 16) |
                         (static_cast<unsigned char>(w3) << 24);
            bias_buf[bias_len++] = packed;
          }
        }
      }
      if (drv.is_bias == 1) {
      ...
      // Send input sums pre-scaled as (sum_x + pK * rhs_offset) * lhs_offset,
      // matching VM handling and avoiding hardware double-scaling.
      for (int i = 0; i < n_remaining; i++) {
        int raw_sum = drv.in_sum[n + i];
        int scaled_sum = (raw_sum + drv.pK * drv.rhs_offset) * drv.lhs_offset;
        bias_buf[bias_len++] = scaled_sum;
      }
