...
SC_MODULE(ACCNAME) {
  ...
  // int cur_crf;
  // int cur_crx;
  // sc_int<32> crf_v[pm_block];
  int crf_v[pm_block];

  sc_int<8> crx_v[pm_block];
  // sc_int<8> crx_v[pmF];
  // int crx_v[pm_block];
  int ra;
  int rhs_offset;
  ...
  sc_int<8> rows[pn_block][1024]; // vit: 768,  deit:
  sc_int<8> cols[pm_block][1024];
  int temp[pkF][4];
  ...
  sc_int<8> dout_3;
  sc_int<8> dout_4;
  int res[pn_block][pm_block];

  int bias[pm_block];
  int wt_sum[pm_block];
  int in_sum[pn_block];
  int prec[pn_block][pm_block];
...
#ifndef __SYNTHESIS__
  // Profiling variables
  // ... (omitted for brevity, as they were not requested, but are in the original file)
...
#ifndef __SYNTHESIS__
  int Quantised_Multiplier_Conv(int, sc_int<32>,
                                sc_int<8>); // Defined in "compute.sc.h"
...
#pragma HLS array_partition variable = crf_v complete dim = 0
#pragma HLS array_partition variable = crx_v complete dim = 0
#pragma HLS array_partition variable = bias complete dim = 0
#pragma HLS array_partition variable = wt_sum complete dim = 0
#pragma HLS array_partition variable = in_sum complete dim = 0
