  ...
  TfLiteStatus Eval(TfLiteContext *context, TfLiteNode *node) override {
    ...
    for (int i = 0; i < node_count; i++) {
      ...
      if (builtin_code_[i] == kTfLiteBuiltinConv2d) { // CONV2D
        ...
        // if(groups > 1) {
        //   cout << "Grouped Convolution with groups: " << groups << endl;
        //   // print out layer id
        ...
        int pad_width = data->padding.width;
        int pad_height = data->padding.height;
        ...
        drv.lhs_offset = filter_offset; // Changing this to 0 does nothing
        drv.a_t = &a_t;
