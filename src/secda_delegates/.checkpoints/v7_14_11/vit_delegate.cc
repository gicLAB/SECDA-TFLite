...
// 
    ...
    // Check if pK would exceed 1024 limit
    if (delegated_node) {
      int K = 0;
      int unroll = 64;
      
      if (isCONV2D) {
        // For CONV2D, K = filter_cols = FlatSizeSkipDim(filter_shape, 0)
        const TfLiteTensor *filter;
        GetInputSafe(context, node->inputs->data[1], &filter);
        RuntimeShape filter_shape = RuntimeShape(filter->dims->size, filter->dims->data);
        K = FlatSizeSkipDim(filter_shape, 0);
      } else if (isFC) {
        // For FC, K = accum_depth = GetTensorShape(filter).Dims(1)
        const TfLiteTensor *filter;
        GetInputSafe(context, node->inputs->data[1], &filter);
        RuntimeShape filter_shape = GetTensorShape(filter);
        K = filter_shape.Dims(1);
      }
      
      // Calculate pK = roundUp(K, unroll) - equivalent to ((K + unroll - 1) / unroll) * unroll
      int pK = ((K + unroll - 1) / unroll) * unroll;
      
      // Don't delegate if pK > 1024
      if (pK > 1024) {
        delegated_node = false;
        std::cout << "Layer with output tensor " << node->outputs->data[0] 
                  << " not delegated: pK=" << pK << " > 1024" << std::endl;
      }
    }
