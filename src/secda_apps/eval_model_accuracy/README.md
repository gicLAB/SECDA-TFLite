...
- We modify the eval_model app to build eval_model_accuracy app.
- This app will give top-1 and top-5 accuracy of the model on the given dataset.
  - In this purpose, we have other app called 'imagenet_image_classification'
  which will give the top-1 and top-5 accuracy of the model on the ImageNet dataset only.
...
```
bazel build -c opt //tensorflow/lite/examples/eval_model:eval_model
```
...
bazel build -c opt --config=android_arm64 \
  //tensorflow/lite/examples/eval_model:eval_model
```
...
```
bazel-bin/tensorflow/lite/examples/secda_apps/eval_model/eval_model \
  --tflite_model /tmp/mobilenet_v1_1.0_224.tflite \
  --labels /tmp/labels.txt \
  --image tensorflow/lite/examples/secda_apps/eval_model/testdata/grace_hopper.bmp
```
...
```
adb push bazel-bin/tensorflow/lite/examples/secda_apps/eval_model/eval_model  /data/local/tmp
adb push /tmp/mobilenet_v1_1.0_224.tflite  /data/local/tmp
adb push tensorflow/lite/examples/secda_apps/eval_model/testdata/grace_hopper.bmp  /data/local/tmp
...
```
adb shell "/data/local/tmp/eval_model \
...
With `-h` or any other unsupported flags, `eval_model` will list supported
options:

```shell
sargo:/data/local/tmp $ ./eval_model -h
./eval_model: invalid option -- h
eval_model
...
See the `eval_model.cc` source code for other command line options.
