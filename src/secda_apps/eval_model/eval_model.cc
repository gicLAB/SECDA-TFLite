...
#include "tensorflow/lite/examples/secda_apps/eval_model/utils/bitmap_helpers.h"
#include "tensorflow/lite/examples/secda_apps/eval_model/utils/get_top_n.h"
#include "tensorflow/lite/examples/secda_apps/eval_model/utils/log.h"
...
void RunInference(Settings *settings,
  ...
  processOutput(interpreter.get(), output, threshold,
                settings->number_of_results, settings);
}
...
int Main(int argc, char **argv) {
  ...
  while (true) {
    ...
    c = getopt_long(argc, argv,
                    "a:b:c:d:e:f:g:i:j:l:m:n:p:r:s:t:v:w:x:", long_options,
                    &option_index);
