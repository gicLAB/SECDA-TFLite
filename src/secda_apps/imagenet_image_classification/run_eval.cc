...
constexpr char kSkipEvaluation[] = "SkipEvaluation";
...
private:
  ...
  void PrintResult(
      const EvaluationStageMetrics &latest_metrics) const; // rpp implementation
...
std::vector<Flag> ImagenetClassification::GetFlags() {
  std::vector<tflite::Flag> flag_list = {
      ...
      tflite::Flag::CreateFlag(
          kSkipEvaluation, &skip_evaluation_,
          "Skip evaluation upto the percentage mentioned.")};
  return flag_list;
}
...
absl::optional<EvaluationStageMetrics> ImagenetClassification::RunImpl() {
  ...
  int start_index = 0;
  for (int i = start_index; i < image_labels.size(); ++i) {
    // TFLITE_LOG(INFO) << "Image no.: " << i;
    if (step > 1 && i % step == 0) {
      TFLITE_LOG(INFO) << "Evaluated: " << i / step << "%";
      TFLITE_LOG(INFO) << "\n";
      if ((i / step) < skip_evaluation_) {
        continue;
      }
      const auto latest_metrics = eval.LatestMetrics();
      OutputResult(latest_metrics);
      // PrintResult(latest_metrics);
    }
    if (step > 1 && (i / step) < skip_evaluation_) {
      TFLITE_LOG(INFO) << "skipping.. ";
      continue;
    }
...
// Original implementation -- cannot print in the text file in a readable format
...
void ImagenetClassification::OutputResult(
  ...
  if (!output_file_path_.empty()) {
    std::ofstream metrics_ofile;
    metrics_ofile.open(output_file_path_, std::ios::out);
    metrics_ofile << "Num evaluation runs: " << latest_metrics.num_runs()
                  << std::endl;
    const auto &metrics =
    ...
    metrics_ofile << "Preprocessing latency: avg="
    ...
    metrics_ofile << "Inference latency: avg=" << inference_latency.avg_us()
...
void ImagenetClassification::PrintResult(
    const EvaluationStageMetrics &latest_metrics) const {
