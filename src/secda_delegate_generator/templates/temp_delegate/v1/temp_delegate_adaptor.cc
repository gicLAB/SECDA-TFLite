...
TfLiteDelegate *CreateTempDelegateFromOptions(char **options_keys,
                                              char **options_values,
                                              size_t num_options) {
  TempDelegateOptions options = TfLiteTempDelegateOptionsDefault();

  // Parse key-values options to TempDelegateOptions by mimicking them as
  // command-line flags.
  std::vector<const char *> argv;
  argv.reserve(num_options + 1);
  constexpr char kTempDelegateParsing[] = "temp_delegate_parsing";
  argv.push_back(kTempDelegateParsing);
  ...
  return TfLiteTempDelegateCreate(&options);
}
...
tflite_plugin_create_delegate(char **options_keys, char **options_values,
  ...
  return tflite::tools::CreateTempDelegateFromOptions(
      options_keys, options_values, num_options);
}

TFL_CAPI_EXPORT void tflite_plugin_destroy_delegate(TfLiteDelegate *delegate) {
  TfLiteTempDelegateDelete(delegate);
}
