...
struct FC_Data {
  ...
  int32_t output_activation_min;
  int32_t output_activation_max;
  // The index of the tempdelorary tensor where the quantized inputs are cached.
...
struct DWCONV2D_Data {
  ...
  // Hybrid per channel tempdelorary tensors.
  int input_quantized_id = kTensorNotAllocated;
  int scaling_factors_id = kTensorNotAllocated;
  int input_offset_id = kTensorNotAllocated;
  int32_t input_quantized_index;
...
struct TCONV_Data {
  ...
  // col2im is the tempdelorary tensor allocated and used in optimized path for
  ...
  // In optimized path, we will transpose them back to HWOI, this tempdelorary
