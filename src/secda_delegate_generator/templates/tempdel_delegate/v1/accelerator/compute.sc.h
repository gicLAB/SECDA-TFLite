...
void ACCNAME::send_parameters_tempdel_PE(int length, sc_fifo_in<ADATA> *din) {
...
void ACCNAME::Compute() {
  ...
  while (1) {
    ...
    send_parameters_tempdel_PE(length, &din1);
    ...
    for (int i = 0; i < length; i++) {
      hw_submodule_array.input_fifo_write(submodule_idx, din1.read().data.to_int());
      hw_submodule_array.input_fifo_write(submodule_idx, din1.read().data.to_int());
      computeS.write(3);
      computeSS.write(3);
      wait();
