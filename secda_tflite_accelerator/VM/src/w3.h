...
#pragma HLS RESOURCE variable=od17ct core=Mul
	...
	while(1){
		...
		ACC_DTYPE<32> i1 = lhsdata1c[l_pointer];
		ACC_DTYPE<32> i2 = lhsdata2c[l_pointer];
		ACC_DTYPE<32> i3 = lhsdata3c[l_pointer];
		ACC_DTYPE<32> i4 = lhsdata4c[l_pointer];

		ACC_DTYPE<32> w1 = rhs3a_1[0];
		ACC_DTYPE<32> w2 = rhs3b_1[0];
		ACC_DTYPE<32> w3 = rhs3c_1[0];
		ACC_DTYPE<32> w4 = rhs3d_1[0];
	    ...
	    for (int rin = 1; rin < d ; rin++){
			...
			i1 = lhsdata1c[rin+l_pointer];
			i2 = lhsdata2c[rin+l_pointer];
			i3 = lhsdata3c[rin+l_pointer];
			i4 = lhsdata4c[rin+l_pointer];
