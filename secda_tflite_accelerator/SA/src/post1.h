...
#pragma HLS array_partition variable=ls cyclic factor=4 dim=0


#pragma HLS array_partition variable=pcrf cyclic factor=4 dim=0
#pragma HLS array_partition variable=pex cyclic factor=4 dim=0
#pragma HLS array_partition variable=pls cyclic factor=4 dim=0
#pragma HLS array_partition variable=prs cyclic factor=4 dim=0
#pragma HLS array_partition variable=msks cyclic factor=4 dim=0
#pragma HLS array_partition variable=sms cyclic factor=4 dim=0


//#pragma HLS array_partition variable=ind cyclic factor=4 dim=0
...
#pragma HLS pipeline II=1
			...
			exar[0] = pex[(i*4)+0]=ex.range(7,0);
			exar[1] = pex[(i*4)+1]=ex.range(15,8);
			exar[2] = pex[(i*4)+2]=ex.range(23,16);
			exar[3] = pex[(i*4)+3]=ex.range(31,24);
...
#pragma HLS unroll
		...
		if (print_po){
			int r = (rb+rhs_block_max)-16;
			int l = (lb+lhs_block_max)-16;
			cout << "------------------------------------------------" << endl;
			cout << "P1:" << "r: " << r << " l: " << l  << endl;
			for(int i=0;i<16;i++){for(int j=0;j<4;j++){
			cout << (int) r1[i*16 + j*4 + 0] << ",";
			cout << (int) r1[i*16 + j*4 + 1] << ",";
			cout << (int) r1[i*16 + j*4 + 2]  << ",";
			cout << (int) r1[i*16 + j*4 + 3] << ",";
			}cout << endl;}
			cout << "------------------------------------------------" << endl;
		}
#endif
