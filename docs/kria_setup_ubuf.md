...
## Step 2: Clone U-DMA Buffer (UBUF) Kernel Module and Manager Source Code
The U-DMA Buffer (UBUF) kernel module is required for the SECDA benchmark suite to function properly. The source code for the UBUF kernel module can be obtained from the Xilinx GitHub repository:

```bash
git clone https://github.com/ikwzm/udmabuf.git
git clone https://github.com/ikwzm/u-dma-buf-mgr.git
```
...
## Step 5: Prepare the Linux Kernel Source for Building UBUF module

```bash
mkdir ./jammy/drivers/staging/u-dma-buf
mkdir ./jammy/drivers/staging/u-dma-buf-mgr
pushd ./udmabuf/
cp Kconfig Makefile u-dma-buf.c ../jammy/drivers/staging/u-dma-buf/
popd
pushd ./u-dma-buf-mgr/
cp Kconfig Makefile u-dma-buf-mgr.c u-dma-buf-funcs.h ../jammy/drivers/staging/u-dma-buf-mgr
popd

nano ./jammy/drivers/staging/Kconfig
# Add the following lines to the end of the file
source "drivers/staging/u-dma-buf/Kconfig"
source "drivers/staging/u-dma-buf-mgr/Kconfig"

nano ./jammy/drivers/staging/Makefile
# Add the following lines to the end of the file
...
## Step 6: Edit the Linux Kernel Configuration
...
Additionally, you can also enable the `CONFIG_U_DMA_BUF` and `CONFIG_U_DMA_BUF_MGR` options in the configuration to build the UBUF kernel module as part of the kernel build process.

4. Go back to the main menu
5. Go to the **Devices Drivers** section
6. Find **Staging drivers** section
7. Find **U-DMA Buffer**
8. Press **Y** to enable them as built-in modules
9. Press enter to expand the **U-DMA Buffer**, enable all as built-in modules, including the **U-DMA Buffer Manager**
...
## Step 8: Install the Linux Kernel Packages
